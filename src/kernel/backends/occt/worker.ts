...
import { chamfer, extrude, fillet, makeCompound, sketchOnFace } from '../../../shared/worker/geometryHelpers';
...
import {
  ...
  type WorkerResponse,
  WorkerRequestSchema,
} from '../../../shared/worker/workerTypes';
...
type ExportRequest = Extract<WorkerRequest, { type: 'EXPORT_STEP' | 'EXPORT_STL' }>;

function evaluateUserCode(code: string): { shapes: unknown[]; activeSketches: SafeSketcher[] } {
  ...
  const shapes = (Array.isArray(result) ? result : [result])
    .map(unwrapV01Shape)
    .filter(Boolean);
  return { shapes, activeSketches };
}

function collectReturnedGeometry(shapes: unknown[]): {
  geometries: GeometryResult[];
  returnedSketches: SketchGeometry[];
} {
  ...
  return { geometries, returnedSketches };
}

function collectTrackedSketches(activeSketches: SafeSketcher[]): SketchGeometry[] {
...
function dedupeSketches(
  returnedSketches: SketchGeometry[],
  trackedSketches: SketchGeometry[],
): SketchGeometry[] {
  ...
  return [...allSketchesByFingerprint.values()];
}

function collectTransferables(
  geometries: GeometryResult[],
  allSketches: SketchGeometry[],
): Transferable[] {
...
async function handleExecute(request: ExecuteRequest): Promise<void> {
  ...
  try {
    ...
    const { shapes, activeSketches } = evaluateUserCode(code);
    const { geometries, returnedSketches } = collectReturnedGeometry(shapes);
    const trackedSketches = collectTrackedSketches(activeSketches);
    const allSketches = dedupeSketches(returnedSketches, trackedSketches);
    const transferables = collectTransferables(geometries, allSketches);
