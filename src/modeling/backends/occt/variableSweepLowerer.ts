...
export function lowerVariableSweep(
  ...
  const { spineWire, firstVertex, lastVertex } = collectSpineAnchor(oc, spineEdge);
  ...
  addProfileSections(pipeShell, sections, firstVertex, lastVertex, withContact, withCorrection);
...
function addProfileSections(
  ...
  sections: VariableSweepSectionLowered[],
  firstVertex: SpineAnchor['firstVertex'],
  lastVertex: SpineAnchor['lastVertex'],
  withContact: boolean,
  withCorrection: boolean,
): void {
