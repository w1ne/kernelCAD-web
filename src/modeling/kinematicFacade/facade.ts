...
// to know the shape of the object it injects at runtime — the layering rule
// (shared -> kernel -> modeling -> kinematic -> agent -> studio) forbids
// modeling from importing kinematic, even for types. The kinematic layer
// implements this interface and registers an implementation via
// `registerKinematicFacade` (see `modeling/api.ts`); `src/kinematic/types.ts`
// re-exports every type below so kinematic's own public surface is
// unchanged. sweepTolerance's types are NOT here — that facade method now
// lives in the agent layer (`src/agent/kinematic/sweepTolerance.ts`) and is
// attached onto the runtime object outside this interface (see
