...
import type { SketchEntity, Point2D, LineEntity, CircleEntity, RectangleEntity } from '../../shared/types/sketch';
import type {
...
export function convertSketchEntitiesToConstraints(sketchEntities: SketchEntity[]): {
    ...
    sketchEntities.forEach(entity => {
        switch (entity.type) {
            case 'line':
                addLineEntity(entity, solverEntities, solverConstraints, createPoint);
                break;

            case 'circle':
                addCircleEntity(entity, solverEntities, solverConstraints, createPoint);
                break;

            case 'rectangle':
                addRectangleEntity(entity, solverEntities, solverConstraints, createPoint);
                break;
        }
    });
...
function addLineEntity(
    entity: LineEntity,
    solverEntities: Map<string, SolverEntity>,
    solverConstraints: Constraint[],
    createPoint: (p: Point2D, idPrefix: string) => string
): void {
