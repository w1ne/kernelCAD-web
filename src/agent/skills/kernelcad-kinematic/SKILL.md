...
2. Every declared mate passes Gate 6 (mate physical realization): the pin/equivalent feature actually constrains the two parts, the pin stays in both holes at every pose in the mate's limits, and bearing surfaces align. Failing this emits `assembly.mate.not-physically-realized` (`error` severity; revolute / prismatic only; `fastened` mates are exempt). `joint.clevis(...)` passes by construction only when its returned geometry and connectors are actually used.
