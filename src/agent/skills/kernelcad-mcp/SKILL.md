...
- `add_nurbs_curve({ code, controlPoints, degree?, weights?, knots?, closed?, binding_name? })` — insert a `nurbsCurve(...)` declaration before the last top-level return. The new binding is the 3D parametric curve consumed by `variableSweep` as a spine (and future `surfaceFromBoundary` in Slice C). Validates Vec3 + control-point count; full validation re-runs via the post-edit evaluateScript pass.
