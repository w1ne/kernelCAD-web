...
- `inspect({ of: 'continuity', file? code?, feature_id?, edges? })` — G0/G1/G2 classification of shared edges: position gap, normal jump, curvature difference, worst-sample XYZ. Box corners are G0; default fillets are G1; a G2 blend classifies G2. `edges` is an EdgeQuery or `@kc[...]` refs.
- `inspect({ of: 'curvature', file? code?, feature_id?, faces? })` — per-face Gaussian and mean curvature min/max/mean, inflections, spikes. Sphere K = 1/r²; cylinder K = 0, |H| = 1/(2r).
