...
- `diff_geometry({ baseFile? baseCode?, file? code?, params?, render? })` — the deeper sibling of `diff_scripts`, for when a volume/bbox delta is ambiguous: a boss that grew and a pocket that deepened report the same |ΔV|, and a part that only moved reports ΔV = 0. The revised side is either another script (`file`/`code`) or the SAME script re-lowered with `params` overrides (declared `param()` name → new value — the one-script form a parameter sweep asks for). Bodies pair by name, falling back to declaration-order positional pairing; leftovers land in `unmatched` and raise `diff.body.unmatched`. Per matched body: `addedMm3` = volume(revised − base), `removedMm3` = volume(base − revised), `commonMm3` = volume(base ∩ revised), exact bbox with min/max/extent deltas, face/edge/hole count deltas, `maxDeviationMm` (two-sided discrete Hausdorff distance), and a `verdict` — `identical` | `moved` | `resized` | `topology-changed` (precedence: topology-changed > resized > moved > identical). **Branch on the verdict; cite the numbers.** `{ render: true }` also writes an overlay PNG — added green, removed red, unchanged material as a translucent ghost — through the `render_preview` pipeline; the scene itself is a re-runnable `diff-overlay.kcad.ts` over lossless BREP sidecars in `render.out_dir`. It fails open: the numeric diff is returned either way, with `render.error` naming what the render pipeline needs. Read-only. `diff_scripts` sets `deeperDiffAvailable` when its own numbers are not enough to say what physically changed; follow that pointer here instead of reaching for a render.
...
- `trace_from_image({ imageUrl, hint?, features?, maxWaypointsPerFeature?, backend? })` — trace pixel-space features from a reference photo into normalized `[0..1]` waypoints feedable to `add_path_segment({ kind: 'spline' })` / `add_path_segment({ kind: 'nurbs' })` after a scale-anchor conversion to mm. Three backends are dispatched: `opencv` (deterministic; uniform-bg silhouette only), `vision-llm` (Claude vision; caller-supplied `ANTHROPIC_API_KEY`), `hybrid` (opencv silhouette + LLM-labeled named points); default `backend: 'auto'` routes by corner-color stddev. Per-feature `confidence` is reported. Pair with the `kernelcad-trace-from-image` sub-skill (under `kernelcad-from-reference/`) for the conversion-to-mm pipeline. The response also carries a `ledger` classifying every fact `visible`/`inferred`/`assumed`/`missing`; resolve open facts with `resolve_assumptions` before building geometry from it.
- `resolve_assumptions({ ledgerPath, resolutions })` — confirms (`{ id, confirm: true }`) or overrides (`{ id, value }`) open facts in a `<model>.ledger.json` assumption ledger written by `trace_from_image` or `drawing_to_cad`. Rewrites the ledger file and returns `{ ok, ledger, paramOverrides, diagnostics }` — feed `paramOverrides` straight into `set_param`.
- `drawing_to_cad({ path? pdfBase64?, page?, projection?, out?, verify? })` — engineering drawing PDF → editable part. Deterministic: reads vector linework (width, dash) and positioned text, classifies visible/hidden/center/dimension/extension lines, reads title-block scale/units/projection symbol, identifies front/top/side views by projection alignment, ties `⌀`/`R`/`4×`/`±`/`THRU` text to its lines (stated values win over drawn lengths), and rebuilds an extruded silhouette or a revolved turned part plus holes. Returns `{ ok, sheet, views, reconstruction, params, script, ledger, fidelity, diagnostics }`; `fidelity.verdict` (match | partial | mismatch | failed) comes from evaluating the script and re-projecting it through the `svg-drawing` view stage (extents, hole diameters, per-view silhouette IoU). `out` writes the script and `<stem>.ledger.json` for `resolve_assumptions`. Diagnostics: `reference.drawing.raster-only` (a scan — use `trace_from_image`), `reference.drawing.view-ambiguous`, `reference.drawing.dimension-unassociated`, `reference.drawing.depth-missing`. See `kernelcad-from-reference` § Engineering drawing PDF.
...
- `project_curve({ code, target, commands, face, scaleMode?, bindAs? })` — insert a `<shape>.projectCurve({ source: { kind: 'sketchCommands', commands: [...] }, face })` chained call before the last top-level return. `commands` is the closed 2D path as plain-number segments (start with `moveTo`, end with `close`). Wraps the curve onto a 3D face along the face normal; returns a `Sketch` — pair with `.extrude(d)` for raised silhouettes or with the parent's `.subtract(...)` for engraved logos. Open-wire `asEdge: true` is not implemented and is rejected at edit time.
...
- `export({ target: 'model', file? | code?, output_path, format, feature_id?, options?, no_verify? })` — write the script geometry to a file server-side. `format` is one of `stl`, `step`, `dxf`, `3mf`, `glb`, `svg-drawing` (the reserved `urdf`, `srdf`, `sdf-gazebo` slots ship in a follow-up slice and return a structured `*.not-implemented` diagnostic today). STL exports run a watertight verify **by default**; a failing mesh still writes the file (so the broken mesh can be inspected) but the call returns `ok: false` with `export.mesh.not-watertight` (open-edge count + up to 5 crack-cluster locations). Pass `{ no_verify: true }` only to skip the gate. Returns `{ ok, output_path, byte_count, feature_count, format, diagnostics }`. `feature_count` is the total features in the script, not the count contributing to the exported shape.
  ...
  **Per-format options** (passed via `options`):
  ...
  - `svg-drawing` — third-angle engineering-drawing sheet (front/top/left + isometric, hidden edges dashed, tangent edges thin, overall bbox dimensions, title block). `sheet?: 'a4' | 'a3'` (default `a4` landscape); `modelName?: string` (title block; defaults to the script file name); `date?: string` (title block; defaults to a placeholder for deterministic output). Accepts single bodies and assembly Scenes — parts project together so inter-part occlusion renders as hidden lines. Pass `exploded: { factor, mode }` to explode the isometric cell (`mode` is `mate-axis` or `radial`); `balloons: true` draws one balloon per BOM unique item (numbers match `inspect({ of: 'bom' })` item numbers); `partsList: true` draws item/name/qty/material above the title block. A script with no assembly fails explode with `render.explode.no-assembly` and warns `drawing.balloons.bom-unavailable` if balloons were requested. See `kernelcad-drawings` for the full sheet anatomy.

  PBR materials propagate from `.finish(name)` and `.material({...})` calls in the script (a finish expands to the same PBR record) through `MeshPhysicalMaterial` into the glTF `KHR_materials_*` extensions (transmission / clearcoat / anisotropy / sheen / volume / ior). 3MF carries the `baseColor` only (the format has no rich PBR slot). STEP carries a flat per-part colour only — finish/PBR surface character is a render-time property and is not written to STEP. DXF carries no material.
...
- `inspect({ of: 'bom', file? | code?, assembly? })` — bill of materials over `assembly.model()`/`solvedModel()`: one row per DISTINCT part, grouped by geometry/catalog identity (never by name) so pattern-placed and repeated-name instances collapse into one row with a real `quantity`. Each row: `{ item, name, quantity, kind: 'fabricated'|'purchased', material, density, massGPerUnit, massGTotal, bboxMm, processHint?, catalog?, instancePaths }`. `material`/`density`/mass are `null` (never a guessed water-density number) when the part has neither `opts.material` nor `opts.density` — flagged by `bom.material.unassigned`. Purchased parts (`lib.fetchPart`/`fetch_part` origin) carry `catalog: { id, vendor, partNumber, source, license }`; missing vendor provenance is flagged by `bom.purchased.catalog-metadata-missing`. Returns `{ ok, rows, totals: { partCount, uniquePartCount, totalMassG }, diagnostics }`. Same numbers as `export({ format: 'bom-csv' | 'bom-json' })` — one computation, two surfaces.
...
- `render_preview({ code? | file?, views?, pose?, focus?, hide?, out_dir?, width?, height?, environment?, no_watermark?, no_mechanism_check?, base_url?, explode? })` — render a script to deterministic PNG views and **look at them**: the visual half of the evaluate → render → inspect → fix loop, with **no studio / dev-server precondition**. Pass exactly one of `{ code }` (inline source; relative imports resolve against a temp dir — use `{ file }` for scripts with relative `lib.fromSTEP(...)` imports) or `{ file }`. Renders the canonical engineering views (`front`, `right`, `top`, `iso`; pass `views: ['iso']` for fastest iteration) plus an optional `pose: '<az>,<el>'` arbitrary camera angle. Pass `explode: { factor, mode }` to pull a multi-part assembly apart (`mode`: `mate-axis` default, along parent mate/joint axes, or `radial` from the assembly centroid) — same mesher as `kernelcad render --explode`; a script with no `assembly()` fails with `render.explode.no-assembly`. The render is served from a prebuilt static player on an ephemeral local port automatically (a running studio dev server is used as fallback; `base_url` forces one) — the only environment dependency is playwright chromium (`npx playwright install chromium`). `focus` / `hide` isolate parts, same semantics as `kernelcad render --focus/--hide`. PNGs land in `out_dir` (default: a fresh temp session dir) and come back as absolute paths with per-view camera descriptions (kernelCAD is Z-up: front looks along +Y, right along -X, top straight down, iso from the front-right-above octant). Mechanism truth runs first with `kernelcad render` parity: a broken mechanism still renders but every tile is watermarked **MECHANISM BROKEN** (`KERNELCAD_RENDER_STRICT=1` refuses instead) — read `{ mechanism, mechanism_failure_codes }`. The probe runs full BREP interference sweeps and can dominate latency on large assemblies — pass `no_mechanism_check: true` for fast visual iteration (the preview then reports `mechanism: 'unverified'`; ignored under strict mode, where the gate always runs). Returns `{ ok, images: [{ name, path, description }], out_dir, bounds, mechanism, render_source, render_ms, diagnostics }`. Paths are local to the machine running the MCP server: local stdio clients read them directly; hosted/remote clients should use `open_in_studio` instead. CLI equivalents: `kernelcad render <file>` (composite), `kernelcad render --explode <factor> --explode-mode radial|mate-axis`, `kernelcad render inspect <file> <outDir>` (manifest bundle + mask/depth/normals channels).

### DFM gates

- `verify({ check: 'dfm', file? | code? })` — run the print-readiness gates declared by `dfmSpec()` in the script: part-pair clearance (exact BREP distance), minimum wall thickness (inward ray sampling), void/channel topology (voxel flood-fill), and with `process: 'fdm'` the FDM printability check (overhangs, bridges, nozzle-relative walls and features, bed contact, bed fit, ranked print orientations). Returns the flattened report `{ ok, clearance, walls, voids, fdm?, timings, diagnostics }` (`fdm[]` only when declared); errors if the script declares no `dfmSpec`. Iteration notes:
