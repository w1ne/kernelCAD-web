...
- `resolve_assumptions({ ledgerPath, resolutions })` — confirms (`{ id, confirm: true }`) or overrides (`{ id, value }`) open facts in a `<model>.ledger.json` assumption ledger written by `trace_from_image` or `mesh_to_features`. Rewrites the ledger file and returns `{ ok, ledger, paramOverrides, diagnostics }` — feed `paramOverrides` straight into `set_param`.
- `mesh_to_features({ file? data?, format?, out?, minIoU?, maxDeviationMm?, maxPasses? })` — rebuild an EDITABLE `.kcad.ts` from an STL (binary/ASCII), OBJ or 3MF of a mostly prismatic mechanical part, instead of the faceted solid `lib.fromSTL` gives. Deterministic and measured: welds and checks the mesh, segments planes and cylinders, picks the extrusion axis, slices each band into exact lines / arcs / circles, snaps near-round values, and emits named `param()`s with a revolve (concentric round stacks) or extruded profiles, `.hole()`/`.holes()` for through, blind and counterbored bores (axial or side-drilled), `.cutout()` pockets, `.fillet()` for constant-radius edge blends (radius measured against the sharp edge, snapped, edges grouped by radius and selected with the shortest exact edge query; tangent corner rounds are made sharp so every round is one fillet feature and the kernel rebuilds the corner patches where rounds meet), and boolean subtractions for bores no drilling feature can reach. It then EVALUATES the script and compares it with the mesh over up to 4 refinement passes. Returns `{ script, ledger, fidelity: { volumeIoU, maxDeviationMm, rmsMm, verdict, thresholds }, unmatchedRegions, features, passes }`. `verdict` is `faithful` only when IoU >= `minIoU` (0.98) AND max deviation <= `maxDeviationMm` (max(0.25 mm, 0.1 % of the bbox diagonal)) AND the mesh is watertight AND nothing is unmatched; `approximate` needs IoU >= 0.85; else `failed`. **Rely on the script's dimensions only when the verdict is faithful**; otherwise read `unmatchedRegions` (freeform or tilted surface the script misses, each with a bbox) and the diagnostics (`reference.mesh.not-watertight`, `reference.mesh.low-fidelity`, `reference.mesh.freeform-region-unmatched`). Dimension facts in the ledger use the param name as id, so `resolve_assumptions` on the `<out>.ledger.json` written with `{ out }` yields `paramOverrides` for `set_param`. Hole `u`/`v` in the script are measured from the entry face centroid as it exists at that point in the chain — keep feature order when editing. The fillet reading is kept only when it measures better than the sharp one. Limits: variable-radius and freeform blends, chamfers, drafts, cones/tori, threads and side bosses are not fitted (they stay in `unmatchedRegions` or `notRepresented`, and the verdict stays approximate); fillets on edges shorter than twice their radius are refused by the kernel; multi-body meshes keep only the largest shell; meshes over `maxTriangles` (300000) are refused. CLI equivalent: `kernelcad reconstruct <mesh> -o <out.kcad.ts>` (`--strict` exits 1 unless faithful).
