...
export const reviewPipelineToolEntries: ToolRegistryEntry[] = [
  ...
  },
  {
    definition: {
      name: 'design_loop',
      description: 'PREFERRED for complex / production / enclosure / gearbox / robot-arm / multi-body builds: iterate evaluate→review/verify→revise until green. Run a CAD design loop over attempt scripts: review_cad each attempt, continue past functional attempts with unresolved warnings, return repair prompts (nextActionPrompt), stop on ok or convergence.escalate. For organic/car bodies set likenessProfile:"automotive" and pass bodyLikeness (or automotive stills on visualReview.checks) — attempts fail closed until body-likeness is publishReady. Do not bypass with one-shot evaluate_script+open_in_studio for complex goals. Optionally write a Studio build record JSON.',
      inputSchema: {
        type: 'object',
        properties: {
