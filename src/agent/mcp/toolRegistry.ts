  ...
  },
  {
    definition: {
      ...
      description:
        ...
        "- 'trim' — insert a `<surface>.trimTo(<by>)` or `<surface>.split(<by>)` call. Pass `surface_binding` (the Surface variable name), `by_binding` (the cutter Surface/Shape/Curve3D variable name), and `op: 'trim'` (discard smaller half) or `op: 'split'` (keep both halves).\n" +
