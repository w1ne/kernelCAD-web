...
interface VendorData {
  ...
  specs: SpecsJson;
  rules: DfmRule[];
  matEntry: CatalogJson['skus'][string];
}

type GeometryResolution =
  | { failure: DfmPreflightOutput }
  | {
      region: Region;
      bendThicknessMm: number;
      bendKFactor: number;
      bends: Bend[];
      pipelineDiagnostics: CompilerDiagnostic[];
      parsedDxfFindings: Finding[];
    };

function collectInputDiagnostics(input: DfmPreflightInput): CompilerDiagnostic[] {
...
function loadVendorData(vendor: string, materialSku: string): VendorData | null {
  // Load vendor data files.
  const vendorDir = join(CATALOG_ROOT, 'vendors', vendor);
...
async function resolveGeometrySource(
  ...
  if (input.file || input.code) {
    ...
    if (!fp.ok || !fp.region) {
      return { failure: { ok: false, findings: [], diagnostics: fp.diagnostics } };
    }
...
export async function dfmPreflightTool(input: DfmPreflightInput): Promise<DfmPreflightOutput> {
  ...
  const geometry = await resolveGeometrySource(input, thicknessMm);
  if ('failure' in geometry) return geometry.failure;
  const { region, bendThicknessMm, bendKFactor, bends, pipelineDiagnostics, parsedDxfFindings } = geometry;
