...
export const GLOBALS: ApiEntry[] = [
  ...
  { name: 'spring', signature: "({ length, coilRadius, wireRadius, turns, axis?, pointsPerTurn?, endStyle?, segments? }) => Shape", description: "Build a helical spring as a circular wire profile swept along a smooth B-spline helix spine, producing one continuous watertight solid. `axis` is 'X' | 'Y' | 'Z' (default 'Z'); `length`, `coilRadius`, `wireRadius`, and `turns` must be positive finite numbers; `coilRadius` must be greater than `wireRadius`. `endStyle: 'closed'` adds short integral end bars contained inside the spring envelope. Use this for visible tension/compression springs instead of torus stacks or hand-authored helix segment boilerplate." },
  { name: 'extrudeRect', signature: '(w, h, height, opts?) => Shape', description: 'Extrude a w-by-h rectangle (XY) by `height` along Z. `opts.faceLabels` maps user label names to canonical face names (top/bottom/left/right/front/back) or FaceQuery descriptors. `opts.twistAngle` (degrees, number or ParamRef) is real twisted-extrude geometry: the profile rotates about the profile origin from bottom to top. Unknown option keys throw feature.invalid-args.' },
  { name: 'extrudeCircle', signature: '(r, height, opts?) => Shape', description: 'Extrude a radius-r circle (XY) by `height` along Z. `opts.faceLabels` maps user label names to canonical face names (top/bottom) or FaceQuery descriptors. `opts.twistAngle` (degrees, number or ParamRef) is real twisted-extrude geometry: the profile rotates about the profile origin from bottom to top. Unknown option keys throw feature.invalid-args.' },
  { name: 'extrudePolygon', signature: '(points, depth, opts?) => Shape', description: 'Extrude a 2D polygon (array of [x, y] points; coordinates and `depth` accept ParamRefs) by `depth` along Z. `opts.faceLabels` maps user label names to canonical face names or FaceQuery descriptors. `opts.twistAngle` (degrees, number or ParamRef) is real twisted-extrude geometry: the profile rotates about the profile origin from bottom to top. Unknown option keys throw feature.invalid-args.' },
  { name: 'extrudeRoundedRect', signature: '(width, height, radius, depth, opts?) => Shape', description: 'Extrude a rounded rectangle (corner radius) by `depth` along Z. `opts.faceLabels` maps user label names to canonical face names (top/bottom/left/right/front/back) or FaceQuery descriptors. `opts.twistAngle` (degrees, number or ParamRef) is real twisted-extrude geometry: the profile rotates about the profile origin from bottom to top. Unknown option keys throw feature.invalid-args.' },
  ...
  { name: 'lib', signature: '{ fromSTEP(path: string): Promise<Shape>; fromBREP(path: string): Promise<Shape>; fromSTL(path: string, opts?: { tolerance?: number; allowOpen?: boolean; maxTriangles?: number }): Promise<Shape>; fromDXF(path: string, opts?: { units?: LengthUnit; tolerance?: number }): Promise<Sketch[]>; fromSVG(path: string, opts?: { units?: LengthUnit; tolerance?: number; curveTolerance?: number }): Promise<Sketch[]> }', description: 'Parts library namespace. Every `from*` call resolves `path` relative to the calling .kcad.ts script (absolute paths also accepted) and returns a Shape that composes with translate/rotate/color/arm.part(...) like any primitive. `lib.fromSTEP(path)` imports STEP — the default for vendor catalog parts (servos, bearings, fasteners) so geometric fidelity matches the real component instead of being hand-authored from box/cylinder. `lib.fromBREP(path)` imports OCCT native .brep: exact analytic surfaces plus full topology with no schema translation and no tessellation, so it is lossless and every downstream op (fillet, chamfer, shell, face queries) is valid — it is the right format for round-tripping kernelCAD geometry between processes, but it is OCCT-specific, so use STEP to interchange with other CAD systems. `lib.fromSTL(path, opts?)` imports an ASCII or binary STL: BE AWARE THIS IS A MESH, NOT ANALYTIC B-REP. The triangles are sewn back into topology and promoted to a solid when they close, so booleans, volume, mass properties, bbox and export all work — but the faces remain planar facets, so a hole that was a cylinder in the source CAD is now a fan of quads, and operations needing analytic surfaces (fillet/chamfer on a "curved" edge, canonical face refs, hole detection) will not behave as they do on STEP/BREP input. Prefer fromSTEP/fromBREP whenever the source is available; when only a mesh exists and you need an editable model rather than facets, run the `mesh_to_features` MCP tool (CLI `kernelcad reconstruct`), which emits a feature-tree script and measures it against the mesh. A non-watertight mesh is REFUSED with `kernel-failed.lib.fromSTL.open-mesh` rather than silently returned as an open shell whose volume and booleans would be undefined; pass `{ allowOpen: true }` to accept the shell deliberately. `{ tolerance }` (default 1e-6 mm) is the vertex-merge distance for sewing — raise it for third-party meshes whose shared vertices are not bit-identical. `{ maxTriangles }` (default 250000) caps import cost, which is roughly linear with a large constant (~10ms at 12 triangles, ~49s at 100k); past the budget the call fails fast with an actionable error rather than stalling. `lib.fromDXF(path, opts?)` and `lib.fromSVG(path, opts?)` import a 2D DRAWING, not a body: both return `Sketch[]` (never a bare Sketch), ordered by DESCENDING enclosed area, so index 0 is the outermost profile and the rest are inner loops — a drawing routinely holds an outline plus its holes and nothing in the file says which loop is a pocket, a through-hole or a separate part, so the caller states the intent: `const [outline, ...holes] = await lib.fromDXF(`plate.dxf`); let p = outline.extrude(3); for (const h of holes) p = p.subtract(h.extrude(3));`. The returned Sketches are ordinary sketch records — identical to `path().moveTo(...).close()` — so `.extrude` / `.revolve` / `.sweep` / `.loft` all apply. fromDXF reads LINE, ARC, CIRCLE, LWPOLYLINE and POLYLINE, mapping the DXF BULGE field (tan(includedAngle/4)) straight onto `bulgeArc` so arcs stay EXACT; SPLINE, ELLIPSE, INSERT and any unrecognised entity are REFUSED with `kernel-failed.lib.fromDXF.unsupported-entity` naming the entity and its line number, because dropping a profile-bearing entity leaves an invisible hole in the result (annotation entities — TEXT/MTEXT/DIMENSION/POINT/… — are skipped and counted in the sketch metadata). fromSVG reads `<path>` (M/L/H/V/C/S/Q/T/A/Z, absolute and relative, including run-together arc flags), `<rect>`, `<circle>`, `<ellipse>`, `<polygon>`, `<polyline>`, `<line>` and `<g>` nesting with translate/scale/rotate/matrix transforms; `<use>`, `<text>` and `<image>` are REFUSED (convert to paths in the source tool). The SVG Y axis points DOWN and is reflected about the top of the `viewBox` so the profile lands upright in positive Y. Lines and circular arcs are exact; Béziers and true ellipses have no exact sketch-command form and are chord-approximated within `{ curveTolerance }` mm (default 0.01). UNITS: DXF uses `$INSUNITS`, and when it is absent or 0/Unitless the coordinates are taken as MILLIMETRES; SVG uses `viewBox` plus a physically-dimensioned `width` (e.g. `width="120mm"`), and without one a user unit is one CSS pixel (1/96 in). Either way the decision is recorded on every returned sketch as `metadata.unitSource` and can be overridden with `{ units }`. Malformed input is reported explicitly: duplicate and exactly-reversed segments are dropped (count reported), gaps smaller than `{ tolerance }` (default 1e-3 mm) are snapped shut (count reported), and anything else fails — an unclosed chain reports both dangling endpoints and the gap size, a junction where three segments meet reports the point rather than guessing which way to turn, and a file with no usable geometry is an error rather than an empty array.' },
  ...
  { name: 'curveBridge', signature: "(a: Curve3D, b: Curve3D, opts: { continuity: 'G1' | 'G2'; ends?: 'end-start' | 'end-end' | 'start-start' | 'start-end'; tension?: number }) => Curve3D", description: 'Infer end points, tangents and (G2) curvature from two existing Curve3Ds and build a degree-5 Hermite blend. Default ends `end-start` joins a.pointAt(1) to b.pointAt(0). `tension` (default 1) scales C\' by τ and C\'\' by τ². Same geometry as `a.bridge(b, opts)`. Throws `feature.curve-bridge.degenerate-end` when the chosen ends coincide or a tangent vanishes.' },
  { name: 'surfaceIntersection', signature: '(a: Shape | Surface, b: Shape | Surface) => Promise<Curve3D[]>', description: 'Exact surface–surface or face–face intersection via OCCT `BRepAlgoAPI_Section`. Returns one Curve3D per section edge, usable as a sweep spine, projectCurve source, or trim boundary. Throws `feature.surface-intersection.none` when the operands do not cut. Await the result (lowers both operands).' },
  ...
  { name: 'kinematic', signature: 'KinematicFacade', description: 'Namespace with in-process feasibility checks an agent can run before declaring a mechanism design done: `kinematic.checkMountingHoleConsistency(arm)` (fastener-side hole agreement; dispatches to the v0.7.4 substrate), `kinematic.checkSweptCollision(arm, opts?)` (sampled-pose collision sweep across declared joint ranges), `kinematic.checkReachable(arm, opts)` (IK reachability — analytical Pieper first, DLS numeric fallback), `kinematic.checkLoadCapacity(arm, opts?)` (closed-form Euler-Bernoulli beam load check), `kinematic.checkStaticHold(arm, opts?)` (gravitational holding torque/force at a sampled pose grid vs each joint\'s declared actuator capacity), `kinematic.sweepTolerance({ code|file, params, gates? })` (re-evaluate the script across a cartesian product of param() values and run the standard gates on each combo). Every entry is sync compute wrapped in async and returns a typed envelope with `source: "local"` and a `diagnostics` array.' },
...
export const SHAPE_METHODS: ApiEntry[] = [
  ...
  { name: 'feaStudy', signature: "(spec: { material: 'mild-steel' | 'aluminum-6061' | 'pla' | 'petg' | 'abs' | 'nylon' | { E: number; nu: number; yield: number }; fixed: FaceQuery | string; loads: Array<{ faces: FaceQuery | string; force: [number, number, number]; name?: string }>; meshSize?: number; minSafetyFactor?: number; name?: string }) => FeaStudyHandle", description: "Declare a linear-static structural study on this shape: where it is held (`fixed`), what pushes on it (`loads`, force in TOTAL newtons over the selected faces), and what it is made of. Declaration-only: registers a virtual record bound to this shape and returns a handle, not a Shape. `fixed` and `loads[].faces` take the same FaceQuery objects or `@kc[...]` refs as the rest of the API; `param()` references inside them stay symbolic and resolve at run time. `meshSize` (mm) defaults to the bounding-box diagonal / 20. Run it with the `run_fea` MCP tool for peak von Mises stress, peak displacement, minimum safety factor against yield, per-region hot spots, mesh-trust flags and stress-heatmap PNGs. Declaring `minSafetyFactor` makes the study an ENFORCEMENT gate: `evaluate` / `evaluate_script` solves it and fails with `fea.safety-factor.below-min` when the margin is not met. Requires the external CalculiX + gmsh toolchain; its absence fails loudly with `fea.solver.unavailable`, never as a pass. Malformed declarations THROW KernelError (`feature.invalid-args`) — a silently-disabled structural gate is worse than a build failure." },
  { name: 'datum', signature: '(label: string, face: FaceQuery) => Shape', description: "Declare datum `label` (one or two capitals, not I/O/Q) on the face `face` resolves to, for `svg-drawing` sheets. Declaration-only: registers a virtual record bound to this shape and returns the same shape, so it chains without joining the geometry. The exporter draws the datum feature symbol; with `options.autoAnnotate` the letter is pinned to that face and the other datums are derived around it. A letter declared twice throws `feature.invalid-args` at capture; a query that misses the exported geometry fails the export with `drawing.datum.unresolved`." },
  { name: 'tolerance', signature: "(spec: { type: 'position' | 'flatness' | 'perpendicularity' | 'parallelism' | 'concentricity' | 'cylindricity'; value: number; face?: FaceQuery; edge?: EdgeQuery; datums?: string[]; modifier?: '⌀' | 'M' | 'S' }) => Shape", description: "Declare a geometric tolerance (feature control frame) on exactly one of `face` / `edge`, for `svg-drawing` sheets. Declaration-only, returns the same shape. With `options.autoAnnotate`, a declared tolerance replaces the automatic tolerance of the same type on the same feature (a grouped hole pattern counts as one feature: a position tolerance on any hole rim of a `4×` group replaces the group's frame); a type the automatic set does not carry is stacked under that feature's callout. Form tolerances (`flatness`, `cylindricity`) take no datums. Malformed declarations throw `feature.invalid-args` at capture; a query that misses the exported geometry fails the export with `drawing.tolerance.feature-unresolved`." },
  { name: 'color', signature: '(name: ColorToken | `#${string}`) => Shape', description: 'Set HUE ONLY: a role color (servo/gear/beam/shaft/plate/pin/frame/tool) or a literal `#rrggbb` hex. Stored on FeatureRecord metadata; renderer resolves via ROLE_PALETTE. It is the cheapest of the three appearance verbs; use it for schematic assembly renders. When you also want surface character (a real material), use `.finish()`; a `.finish()`/`.material()` PBR record overrides a `.color()` hue on the same leaf. Booleans drop the color (identity lives at leaf parts).' },
  { name: 'finish', signature: '(name: FinishToken | MaterialGrade, opts?: { color?: string; face?: string }) => Shape', description: 'PREFERRED way to make a part look like a real material. An engineering material grade (`mild-steel`, `aluminum-6061`, `nylon`, alias `aluminum`, …) applies that grade\'s default finish. Choose from the named finishes: metals (aluminium, aluminium-brushed, anodized-black, anodized, steel, stainless, brass, copper, titanium), plastics (abs, pla, nylon, delrin, polycarbonate, rubber, plastic-glossy), glass (glass, glass-tinted, acrylic), paints/neutral (paint-matte, paint-gloss, default). Each token expands to the SAME `metadata.material` PBR record `.material()` writes, so renderer + exporters are unchanged. `opts.color` overrides the hue while keeping the finish (use it for paints/ABS/anodising). `opts.face` scopes to an upstream `faceLabels` face, same as `.material({ face })`. Unknown name throws `feature.finish.unknown-token` listing the valid finishes (no silent default). Identity dies at booleans (leaf-part level, same as `.color()`).' },
  { name: 'material', signature: '(opts: PBRMaterial & { face?: string }) => Shape', description: 'ADVANCED / renderer-level escape hatch exposing raw PBR floats for glass, clearcoat, anisotropy, and image textures when no `.finish()` token fits. Prefer `.finish(name)` for a named material; reach here only when you need `transmission`/`thickness`/`attenuation*`, `anisotropy*`, `textures`, or a bespoke clearcoat. baseColor required; optional metalness/roughness/clearcoat/clearcoatRoughness/ior/transmission/sheen/opacity. Numeric fields clamped to [0, 1] (ior: [1.0, 2.5]); out-of-range values emit a `feature.material.value-clamped` soft warning. Use opacity < 1 for clear overlays such as glass when transmission blur is not desired. Identity dies at booleans (leaf-part level, same as `.color()`). Pass `face: "<label>"` to apply only to faces matching a label declared upstream via `faceLabels`; calls accumulate, last write wins on the same label, and a call without `face` sets the shape-level default. Labels that do not resolve emit a soft `feature.material.face-label-no-match` warning and fall back to the default.' },
  { name: 'wrapTexture', signature: "(imageRef: TextureRef, projection: { type: 'flat'; onto?: 'xy' | 'xz' | 'yz' } | { type: 'cylinder'; axis: [number, number, number] } | { type: 'sphere' } | { type: 'box' }) => Shape", description: "Project a bitmap (label, decal, logo) onto this shape without hand-authoring UVs. `imageRef` is the same `{ path, repeat?, offset?, rotation? }` TextureRef as `.material({ textures: { albedo } })`. `projection` is `{ type: 'flat', onto? }`, `{ type: 'cylinder', axis }` (cans, tubes), `{ type: 'sphere' }`, or `{ type: 'box' }`. UVs are derived at mesh/export time from final world-space vertices, so later translates/rotates/booleans do not stale the wrap. Render-only: geometry is unchanged. Last-write-wins on the same `metadata.material` slot as `.finish()` / `.material()`; call wrapTexture first, then `.material({...})` for extra PBR floats. Identity dies at booleans." },
  ...
  { name: 'hole', signature: '(face: FaceSelector | string, opts: { u, v, diameter, depth?: number | "through", upToFace?: FaceRef, counterbore?: { diameter, depth }, countersink?: { diameter, angleDeg? }, thread?: { pitch, modeled?: boolean, clearance? } }) => Shape', description: 'Drill a single hole. Position is face-local 2D (u, v in mm). Use `depth: "through"` to clip at the back face. Optional `counterbore` (wider shoulder) or `countersink` (a real cone, rim `diameter` at the entry face) — mutually exclusive. Optional `thread` makes an ISO metric tapped hole: `diameter` is then the nominal size, the bore is the ISO minor diameter (+ clearance), `modeled: true` cuts the right-hand 60° groove (default false = cosmetic thread, bore only), `clearance` ≤ pitch/8 adds play. A sub-feature that cannot be built fails the hole with an error diagnostic. Created refs: `wall` always, `floor` (blind), `wall-back` (through), `counterbore-wall` / `counterbore-floor` (with cb), `countersink-cone` (with csk).' },
  { name: 'holes', signature: '(face: FaceSelector | string, opts: { positions: Array<{u,v}>, diameter, depth?, upToFace?, counterbore?, countersink?, thread? }) => Shape', description: 'Drill N holes in one feature record. All holes share diameter / depth / cb / csk / thread. Bare `wall` selector on the result resolves to all bore walls collectively (sugar for fillet-all-bore-lips). For mixed specs, chain `.hole()` calls.' },
  ...
  { name: 'sectionSketch', signature: `(plane: 'xy' | 'xz' | 'yz' | { plane, offset? } | { origin, normal }, opts?: { curveTolerance? }) => Promise<Sketch>`, description: 'AWAIT THIS — returns Promise<Sketch>; chaining a Sketch method (e.g. .extrude) directly on the un-awaited call throws feature.async-result.missing-await. Derive a reusable 2D Sketch from an exact planar cross-section of this solid (OCCT BRepAlgoAPI_Section). plane is a cardinal name, { plane, offset }, or any { origin, normal }. Circular section curves are exact bulgeArc commands; a boss with a through-hole returns ONE sketch whose first loop is the outer boundary and whose remaining loops are holes, so a single .extrude(d) yields the solid section. Use it to take an imported STEP / boolean result back to 2D and extrude a matching gasket, then .offset() or dimension it. Throws feature.section.plane-misses-body when the plane does not intersect the body. Async (lowers at capture time).' },
  { name: 'faceSketch', signature: `(face: FaceSelector | string, opts?: { curveTolerance? }) => Promise<Sketch>`, description: "AWAIT THIS — returns Promise<Sketch>; chaining a Sketch method (e.g. .extrude) directly on the un-awaited call throws feature.async-result.missing-await. Derive a Sketch from the boundary of a planar face: its outer wire plus any hole wires, placed in the face's own 2D frame. face takes the same selector as .shell()/.hole() — canonical name ('top'), label, FaceQuery ({ atZ: 5 }), or @kc[owner/face/name] ref. Non-planar faces throw feature.face-sketch.non-planar. Round-trips: faceSketch(face).extrude(t) re-creates the cross-section prism. Async (lowers at capture time)." },
  { name: 'silhouette', signature: `(direction?: [number, number, number], opts?: { curveTolerance? }) => Promise<Sketch>`, description: "AWAIT THIS — returns Promise<Sketch>; chaining a Sketch method (e.g. .extrude) directly on the un-awaited call throws feature.async-result.missing-await. Derive a closed Sketch outline from this solid's orthographic silhouette viewed along `direction` (OCCT HLR). [0,0,1] is the top view; [1,0,0] the right view. A cylinder along its axis silhouettes as a circle, across its axis as a rectangle. Straight edges are exact; curved contours are HLR's deterministic discretization. Recentred on the projected bbox centre. Async (lowers at capture time)." },
];
...
export const SKETCH_METHODS: ApiEntry[] = [
  { name: 'extrude', signature: '(depth: Editable<number>, opts?: { faceLabels?, twistAngle? }) => Shape', description: 'Extrude this closed sketch normal to its plane by `depth` (mm). `depth` may be a number or a `ParamRef` (e.g. `param(\'thickness\', 8)`), so the extrusion depth stays editable with set_param. `opts.twistAngle` (degrees, number or ParamRef) is real twisted-extrude geometry — the profile rotates about the sketch origin from bottom to top, so a profile that does not touch the origin changes its bounding box. Face-bound extrudes (projectCurve output) reject a non-zero twist with feature.invalid-args, as does any unknown option key.' },
  { name: 'revolve', signature: '(opts?: { angleDeg?: Editable<number> }) => Shape', description: 'Revolve around the Z axis, 360 degrees unless `angleDeg` (number or ParamRef). Profile coords are (radial-X, axial-Z); all x >= 0.' },
  { name: 'sweep', signature: '(rail, opts?: { frenet?, transitionMode?, spine? }) => Shape', description: "Sweep this profile along a 3D rail. `spine: 'smooth'` builds a single B-spline spine through the rail points and places the profile at the rail start — required for rails that sample a smooth curve (helix(...), threads, organic paths); the default `spine: 'polyline'` keeps real corners (pipe runs, L-bends) but on dense smooth rails produces per-segment tubes that fail the watertight export verify. `spine: 'helix'` (rails from helix(...) only) sweeps along the EXACT helix by screw motion with the profile read in the axial plane (x = radial offset from the helix point, y = axial offset) — use it for threads: a 60° ISO V profile builds a valid solid; the profile's axial extent must stay below the pitch. `transitionMode: 'right' | 'transformed' | 'round'` picks how polyline corners are bridged. `frenet: true` rotates the profile with the rail curvature." },
  { name: 'loft', signature: '(other: Sketch | Sketch[], opts?: { spacing?, planes?, twistDeg?, twistCenter?, ruled?, startPoint?, endPoint?, faceLabels? }) => Shape', description: 'Loft this profile through one or more additional sections to produce a 3D solid that smoothly interpolates between them. Use for nozzles (round-to-square), wings/airfoils, fairings, transition pieces, twisted blades. `opts.spacing` stacks sections on XY planes at z = 0, spacing, 2·spacing, …; of the rotation controls only `twistDeg`/`twistCenter` apply there (per-plane `rotationDeg` requires `opts.planes`). `opts.planes` overrides with explicit per-section placement and each entry may add `rotationDeg` to rotate that section in-plane about `twistCenter` (default [0, 0]). `opts.twistDeg` distributes an even total twist across the sections (first at 0, last at `twistDeg`); an explicit `planes[].rotationDeg` overrides the distributed value for its section only. Extrude\'s twist option is `twistAngle`; loft\'s is `twistDeg`. `spacing`, plane origins, rotation angles, twist and start/end points accept ParamRefs. Unknown option keys throw feature.invalid-args.' },
  { name: 'reflect', signature: `(axis: 'x' | 'y' | { axis: 'x' | 'y'; offset: Editable<number> }) => Sketch`, description: "Reflect this sketch's path across an axis, returning a new Sketch. 'x' negates y-coords; 'y' negates x-coords; { axis, offset } reflects across the parallel axis at the given offset. Arc winding (signed sagitta/bulge/radius) is inverted automatically. Labels are preserved. ParamRef coordinates and a ParamRef offset stay symbolic in the reflected sketch." },
  ...
  { name: 'loft', signature: '(other: Sketch | Sketch[], opts?: { spacing?, planes?, twistDeg?, twistCenter?, ruled?, startPoint?, endPoint?, faceLabels?, rails?: Curve3D[] }) => Shape', description: 'Loft this profile through one or more additional sections to produce a 3D solid that smoothly interpolates between them. Use for nozzles (round-to-square), wings/airfoils, fairings, transition pieces, twisted blades, rail-guided handles. `opts.spacing` stacks sections on XY planes at z = 0, spacing, 2·spacing, …; `opts.planes` overrides with explicit per-section placement and each entry may add `rotationDeg`; `opts.twistDeg` distributes an even total twist across the sections and `twistCenter` sets the in-plane rotation center (default [0, 0]). Extrude\'s twist option is `twistAngle`; loft\'s is `twistDeg`. `opts.rails` (1 or 2 Curve3Ds) constrains the loft via `BRepOffsetAPI_MakePipeShell` (first rail = spine, second = auxiliary); a rail that misses a section or a third rail emits `feature.loft.rail-miss`, and rail-guided lofts reject twistDeg/rotationDeg with feature.invalid-args and do not apply plane origins to NURBS sections.' },
...
export const PATH_BUILDER_METHODS: ApiEntry[] = [
  ...
  { name: 'tangentCircle', signature: "(entities: TangentEntity2D[], opts?: { radius?: Editable<number>; near?: [Editable<number>, Editable<number>] }) => Sketch", description: "Closed circle TANGENT to other 2D geometry, solved by OCCT Geom2dGcc. Pass 2 entities + opts.radius for the sketch-fillet move (Geom2dGcc_Circ2d2TanRad), or 3 entities and no radius (Geom2dGcc_Circ2d3Tan). An entity is { kind: 'line', from: [x,y], to: [x,y] } (treated as an INFINITE line; from->to also sets its direction) or { kind: 'circle', center: [x,y], radius }, each with an optional side: 'outside' (default) | 'enclosed' | 'enclosing' | 'unqualified'; coordinates and radii accept ParamRefs. Points are NOT supported — the bundled OCCT does not bind Handle_Geom2d_Point. These constructions have SEVERAL solutions (a radius-r circle tangent to two perpendicular lines has four, one per quadrant): `side` is the primary filter, `opts.near: [x,y]` then picks the solution with the closest centre, and if more than one still survives the build FAILS with sketch.tangency.ambiguous listing every candidate rather than guessing. If no such circle exists it fails with sketch.tangency.no-solution naming the geometric reason. Terminal like circle(): must be the only operation on a fresh path, returns a Sketch. The loop is two exact semicircular arcs, not a polyline." },
  { name: 'tangentLine', signature: '(a: TangentEntity2D, b: TangentEntity2D, opts?: { near?: [Editable<number>, Editable<number>] }) => PathBuilder', description: "Straight segment along the line TANGENT to two circles (OCCT Geom2dGcc_Lin2d2Tan) — the belt/pulley move. Both entities must be circles ({ kind: 'circle', center: [x,y], radius, side? }); a line tangent to a line is that same line, so lines are rejected. The segment spans the two tangency points, so chaining tangentLine with arcs around each pulley closes a belt profile. On a fresh path the pen starts at the first tangency point; after an existing segment a straight connecting line is inserted first (it does not teleport). Two circles admit up to four tangent lines: side:'outside' on both selects an external tangent, opts.near then picks the line passing closest to that point, and a still-ambiguous construction fails with sketch.tangency.ambiguous rather than guessing." },
  ...
  { name: 'circle', signature: '(cx: Editable<number>, cy: Editable<number>, r: Editable<number>, segments?: number) => Sketch', description: 'Closed circle profile centered at (cx, cy) with radius r. Polyline approximation (default 48 segments). Must be the ONLY operation on a fresh path. Returns Sketch directly. cx, cy, r accept ParamRefs and stay editable; segments is a plain integer.' },
...
export const CURVE3D_METHODS: ApiEntry[] = [
  ...
  {
    name: 'bridge',
    signature: "(other: Curve3D, opts: { continuity: 'G1' | 'G2'; ends?: 'end-start' | 'end-end' | 'start-start' | 'start-end'; tension?: number }) => Curve3D",
    description: 'Quintic Hermite blend from this curve to `other`. Infers point, tangent and (when continuity is G2) curvature at the chosen ends. Default `ends` is `end-start`. Same geometry as the `curveBridge` global.',
  },
];
