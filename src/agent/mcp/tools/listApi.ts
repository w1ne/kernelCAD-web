...
export const SHAPE_METHODS: ApiEntry[] = [
  ...
  { name: 'projectCurve', signature: `(opts: { source: ProjectCurveSource; face: FaceSelector | string; scaleMode?: 'original' | 'native' | 'bounds'; asEdge?: boolean }) => Shape`, description: 'Wrap a 2D closed curve onto a 3D face along the face normal. Pair with `.extrude(d)` / `.cut(...)` for engraved logos or label inserts on curved bodies (bottle silhouette, helmet badge). `asEdge: true` is captured but currently deferred at lower time — the bundled OCCT does not export BRepProj_Projection.' },
