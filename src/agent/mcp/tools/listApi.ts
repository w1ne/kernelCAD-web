...
export const GLOBALS: ApiEntry[] = [
  ...
  { name: 'setRenderEnvironment', signature: "(spec: { preset?: 'studio' | 'softbox' | 'neutral' | 'outdoor' | 'warehouse'; url?: string; intensity?: number; rotation?: number }) => RenderEnvironmentHandle", description: "Set the HDRI / IBL lighting environment for the rendered scene. Pass exactly one of `preset` (built-in PolyHaven CC0 HDRI) or `url` (a custom `.hdr` served by the dev server). `intensity` (default 1.0) scales `envMapIntensity` on every PBR material; `rotation` (default 0, degrees around world Y) re-aims the env map. Default behavior when the script never calls this is the existing three-light rig — strict backward compatibility with every existing gallery hero render. The record is virtual — no OCCT geometry is produced; the renderer reads it directly. Scene background stays flat #909090; the env map only affects shading. Validation errors (conflicting/missing/unknown preset, intensity out of range) are pushed as structured diagnostics on `handle.metadata.diagnostics` rather than thrown." },
  ...
  { name: 'surfaceFromBoundary', signature: '(curves: [Curve3D, Curve3D, Curve3D, Curve3D], opts?: { continuity?: "C0" | "C1" | "C2" | ("C0" | "C1" | "C2")[]; sampling?: number }) => Surface', description: 'Fill the interior of 4 boundary curves with a single NURBS face (Coons-style patch). Lowers to `BRepOffsetAPI_MakeFilling` (direct OCCT) with `Add_1(edge, GeomAbs_Cn, isBound=true)` per boundary. The 4 curves walk an ordered loop: `curves[0]` = bottom, `curves[1]` = right, `curves[2]` = top, `curves[3]` = left. Adjacent endpoints must coincide within 1e-6 mm (kernel emits `feature.surface-from-boundary.corner-mismatch` otherwise). `opts.continuity` accepts a single grade applied to all 4 edges or a length-4 array per edge; defaults to `"C0"`. `opts.sampling` controls `NbPtsOnCur` (default 15). Returns a Surface peer — chain `.thicken(t)` or `.toShape()` to enter the Shape pipeline.' },
