...
export const GLOBALS: ApiEntry[] = [
  ...
  { name: 'kinematic', signature: 'KinematicFacade', description: 'Namespace with four in-process feasibility checks an agent can run before declaring a mechanism design done: `kinematic.checkMountingHoleConsistency(arm)` (fastener-side hole agreement; dispatches to the v0.7.4 substrate), `kinematic.checkSweptCollision(arm, opts?)` (sampled-pose collision sweep across declared joint ranges; T2 stub, T3 wires the loop), `kinematic.checkReachable(arm, opts)` (IK reachability — T2 stub returns `kinematic.solver.unsupported-config` until T4/T5 ship analytical + numeric solvers), `kinematic.checkLoadCapacity(arm, opts?)` (closed-form beam load check; T2 stub, T6 wires the equation). Every entry is sync compute wrapped in async and returns a typed envelope with `source: "local"` and a `diagnostics` array.' },
];
