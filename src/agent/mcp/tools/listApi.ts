...
export const SKETCH_METHODS: ApiEntry[] = [
  { name: 'extrude', signature: '(depth: Editable<number>) => Shape', description: 'Extrude this closed sketch normal to its plane by `depth` (mm). `depth` may be a number or a `ParamRef` (e.g. `param(\'thickness\', 8)`), so the extrusion depth stays editable with set_param.' },
  ...
  { name: 'loft', signature: '(other: Sketch | Sketch[], opts?: { spacing?, planes?, ruled?, startPoint?, endPoint?, rails?: Curve3D[] }) => Shape', description: 'Loft this profile through one or more additional sections to produce a 3D solid that smoothly interpolates between them. Use for nozzles (round-to-square), wings/airfoils, fairings, transition pieces, rail-guided handles. `opts.spacing` z-stacks sections axially; `opts.planes` overrides with explicit per-section placement. `opts.rails` (1 or 2 Curve3Ds) constrains the loft via `BRepOffsetAPI_MakePipeShell` (first rail = spine, second = auxiliary); a rail that misses a section or a third rail emits `feature.loft.rail-miss`.' },
