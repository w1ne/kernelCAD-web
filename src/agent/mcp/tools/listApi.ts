...
export const SHAPE_METHODS: ApiEntry[] = [
  ...
  { name: 'excludeFromCameraFit', signature: '() => Shape', description: 'Tag this shape so the renderer\'s auto-framer ignores its bounding box when computing camera position. Use for parts that extend the silhouette far beyond the main form (eyewear temples, antennae, decorative spires) where including them in scene bounds tanks framing. Stored on FeatureRecord metadata; identity dies at booleans (re-apply on the union result if needed).' },
