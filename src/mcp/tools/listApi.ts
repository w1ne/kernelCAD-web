...
export const GLOBALS: ApiEntry[] = [
  ...
  { name: 'assembly', signature: '(name?) => Assembly', description: 'Start an inspectable mechanical assembly. Use `.part(name, shape, { at?, connectors?, connect? })` to wrap modeled solids, `.connect(name, aConnector, bConnector)` for fixed connector metadata, joint primitives `.revolute/.prismatic/.fixed/.ball(name, parentPart, childPart, opts)` to declare DOF + joint origins (numeric Vec3 in parent local frame), `.solve(poses)` to run body-tree forward kinematics returning a SolvedKinematics handle (with `.transform(partName)`, `.value(jointName)`, `.bodies()`, `.toShape()`), `.solvedModel(poses)` as sugar for the unioned posed Shape, and `.model()` for the kinematic-zero Shape. Pose values are numeric (no Editable in v1).' },
...
export const SHAPE_METHODS: ApiEntry[] = [
  ...
  { name: 'scale', signature: '(factor: number | [number, number, number]) => Shape', description: 'Scale this shape uniformly (single positive finite number) or per-axis (Vec3 — sx/sy/sz) so face refs survive (topology is preserved under any affine transform). Capture stores per-axis components on the transform record. NOTE: the active OCCT WASM build cannot lower truly non-uniform scale yet (no BRepBuilderAPI_GTransform); non-uniform Vec3s emit a kernel-failed diagnostic with hint kernel-failed.scale.non-uniform — bake the per-axis sizing into primitive dimensions until that ships.' },
