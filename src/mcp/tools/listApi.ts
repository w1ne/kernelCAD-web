...
export const GLOBALS: ApiEntry[] = [
  ...
  { name: 'assembly', signature: '(name?) => Assembly', description: 'Start an inspectable mechanical assembly. Use `.part(name, shape, { at? })` to wrap modeled solids, `.revolute(name, a, b, { axis, origin, limitsDeg? })` to capture joint intent, and `.model()` to return one fused/exportable Shape containing all placed parts. Full kinematic solving is not implied.' },
