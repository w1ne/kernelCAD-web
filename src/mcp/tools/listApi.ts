...
export const GLOBALS: ApiEntry[] = [
  ...
  { name: 'assembly', signature: '(name?) => Assembly', description: 'Start an inspectable mechanical assembly. Use `.part(name, shape, { at?, connectors?, connect? })` to wrap modeled solids, `.connect(name, aConnector, bConnector)` for fixed connector metadata, joint primitives `.revolute/.prismatic/.fixed/.ball(name, parentPart, childPart, opts)` to declare DOF + joint origins (numeric Vec3 in parent local frame), and `.model()` to return one fused/exportable Shape containing all placed parts at kinematic-zero. Body-tree forward kinematics + posed-model rendering ship in a separate slice (assembly.solve / assembly.solvedModel).' },
