...
function getMeta(session: CaptureSession): RenderEnvironmentMetadata & { diagnostics?: CompilerDiagnostic[] } {
  ...
  return rec.metadata as unknown as RenderEnvironmentMetadata & { diagnostics?: CompilerDiagnostic[] };
}
...
describe('CaptureSession.addRenderEnvironment', () => {
  ...
  it('emits diagnostic when both preset and url are set', () => {
    ...
    const meta = getMeta(s);
    expect(meta.diagnostics?.[0]?.code).toBe('feature.render-environment.conflicting-spec');
  });
  ...
  it('emits diagnostic when neither preset nor url is set', () => {
    ...
    const meta = getMeta(s);
    expect(meta.diagnostics?.[0]?.code).toBe('feature.render-environment.missing-spec');
  });
  ...
  it('emits diagnostic when preset key is invalid', () => {
    ...
    const meta = getMeta(s);
    expect(meta.diagnostics?.[0]?.code).toBe('feature.render-environment.unknown-preset');
  });

  it('emits diagnostic when intensity is out of range', () => {
    const s = new CaptureSession();
    s.addRenderEnvironment({ preset: 'studio', intensity: -1 });
    const meta = getMeta(s);
    expect(meta.diagnostics?.[0]?.code).toBe('feature.render-environment.intensity-out-of-range');
  });

  it('records both invocations when called twice (renderer applies last)', () => {
    const s = new CaptureSession();
