...
describe('golden mock replay', () => {
  it('includes nested photo-device asset policy in the aggregate prompt', () => {
    const skillMd = loadCombinedSkillMd();

    expect(skillMd).toContain('name: photo-to-device');
    expect(skillMd).toContain('An active Studio/server uploaded photo is managed');
  });
