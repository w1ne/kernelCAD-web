// ISO metric hex bolt threaded into its hex nut (M3–M12 table). Example is M6:
// pitch 1.0, AF 10, head 4.0, nut 5.2. The bolt carries a real 60° ISO V-thread
// swept along an exact helix; the nut carries the matching internal thread cut
// by hole({ thread }). The nut sits on the bolt in phase, with `threadClearance`
// of play, so the two parts do not interfere.
//
// Run with:
