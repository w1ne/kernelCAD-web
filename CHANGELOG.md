...
- **`svg-drawing` feature dimensions + GD&T annotation kinds.** `export({ format: 'svg-drawing', options: { annotations } })` gains five feature-aware annotation kinds alongside the existing generic ones: `hole` (standard callouts — `⌀6.5 THRU`, blind `⌀6.5 ▾ 10`, `⌴`/`⌵` counterbore/countersink suffixes, `<count>× ` pattern prefix), `fillet` (`R<r>`), `chamfer` (`<size> × <angle>°`, leg size author-supplied — it isn't recoverable from a bare edge query), `datum` (ASME square datum-feature symbol + leader), and `fcf` (feature control frame: type symbol · toleranced value · datum references, for `position`/`flatness`/`perpendicularity`/`parallelism`/`concentricity`/`cylindricity`). The existing `linear` kind gains `tol` (`± 0.1`, `+0.2/−0.05`, or a fit class string like `'H7'`). All five share the existing per-view leader-rotation stacking, so they fan out from other callouts on the same view automatically. 4 new diagnostic codes: `drawing.datum.unresolved`, `drawing.tolerance.feature-unresolved`, `drawing.section.plane-misses-body` (reserved for a future section-view slice), `drawing.annotation.overlap` (reserved, warn). Catalogue grows from 248 → 252. Section views and a general text-box collision solver are not in this slice.
