...
- **USD Isaac robot export (`format: 'usd-isaac'`).** `export({ target: 'model', format: 'usd-isaac' })` writes an ASCII USD (`.usda`) stage using the UsdPhysics schema — a `PhysicsArticulationRootAPI` root, one rigid-body prim per link (`PhysicsRigidBodyAPI` + `PhysicsMassAPI` sourced from the same analytic mass-properties pipeline as `urdf`/`sdf-gazebo`), and one `PhysicsFixedJoint`/`PhysicsRevoluteJoint`/`PhysicsPrismaticJoint` per `fastened`/`revolute`/`prismatic` mate, with a sibling `meshes/<part>.stl` per link (same convention as `urdf`/`sdf-gazebo`). For Isaac Sim / Isaac Lab robot-learning workflows that consume USD directly. Mate kinds with no `PhysicsJoint` equivalent (`planar`/`cylindrical`/`pin_slot`/`ball`) fail closed with `export.usd.joint-unsupported`; a link with non-finite/non-positive mass fails closed with `export.usd.mass-missing`.
