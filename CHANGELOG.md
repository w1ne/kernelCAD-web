...
- **`.rotateX(deg)` / `.rotateY(deg)` / `.rotateZ(deg)` (#438).** Cardinal-axis rotation aliases on `Shape` — thin sugar over `.rotate(axis, degrees, pivot?)` with identical validation, transform records, and ParamRef support. Advertised in `list_api` and the authoring skill so agents discover them before guessing signatures.
