...
- **`repair_script` covers five more error kinds.** Geometry-derived candidates for `feature.revolve.crosses-axis` (clamp path x to 0), `sketch.tangency.no-solution` (radius = (centre-distance − r1 − r2) / 2 for two circles), `feature.draft.failed` (8°/5°/3° ladder), `feature.kernel-failed` on `.shell()` (thickness under half the thinnest bbox dim), and `feature.emboss-text.boolean-noop` (slide UV anchor onto remaining solid). Broken fixtures under `examples/repair/`; `npx tsx examples/repair/run-repair-example.ts`.
- **G-code export forwards the printer bed to OrcaSlicer.** `sliceStlToGcode` now passes `--printable-area` and `--printable-height` from the resolved profile (`generic-fdm` is 220×220×250 mm). A 180 mm column slices; a 300 mm column still fails `export.gcode.exceeds-bed`. Example: `examples/print-loop-tall-column.kcad.ts`.

- **FDM printability check — `dfmSpec({ process: 'fdm', buildDirection?, nozzleMm?, maxOverhangDeg?, maxBridgeMm?, printer? })`.** Design-for-additive analysis before slicing, from the part's real mesh and enforced through the same evaluate seam as the other `dfmSpec` gates. Per printed part: unsupported overhang area steeper than `maxOverhangDeg` (default 45°) relative to the build direction, with bed faces excluded and regions that bridge between supports, sit in the first layers, or reach under one nozzle past their support not counted; the largest region is named by `@kc[...]` face ref and bbox. Also: flat bridges longer than `maxBridgeMm` (default 10), walls under 2 × `nozzleMm` (the existing min-wall sampler, now with an opt-in knife-edge filter so a cone rim is not a wall), holes under 5 × and pins under 7.5 × nozzle, bed contact under 10 % of the footprint, tip risk (height over 8 × base width), and bed fit on the printer profile. All six axis-aligned build directions are ranked (fits bed → unsupported area → bed contact → height), and the hint names the rotation and what it buys: `Rotate -90° about Y (.rotateY(-90), or dfmSpec buildDirection: '+x'): unsupported area drops from 938.2 to 0.0 mm²`. The report is the new `fdm[]` block of `verify({ check: 'dfm' })` and `kernelcad dfm --json`; the CLI summary line gains the unsupported area and best orientation.
- **gcode export slices in the declared build orientation.** With an FDM `dfmSpec`, `export({ format: 'gcode' })` rotates the part into `buildDirection` before its bed gate and the slicer, so the orientation that passed the check is the one that gets sliced. The bed gate and the FDM fit check share one `exceedsBed()` rule and the printer profile data (moved to node-free `printerProfiles.ts` so capture-time validation can read it).
- **7 new diagnostic codes** (`dfm` group): `dfm.fdm.overhang-unsupported`, `dfm.fdm.bridge-too-long`, `dfm.fdm.wall-below-nozzle`, `dfm.fdm.exceeds-bed` (error); `dfm.fdm.feature-too-small`, `dfm.fdm.bed-contact-low`, `dfm.fdm.tip-risk` (warn). Catalogue grows 276 → 283.
- **Example + cookbook.** `examples/print-prep/wall-hook.kcad.ts` fails as modeled on a 938 mm² cantilevered arm; `wall-hook-on-side.kcad.ts` declares the recommended `buildDirection: '+x'`, passes, and slices 24 mm tall. Cookbook entry `fdm-printability-and-print-orientation`; `kernelcad-print`, `kernelcad-authoring`, `kernelcad-mcp`, and `kernelcad-shopcheck` skills updated.
- **Sketch from shape — derive reusable 2D profiles from a solid, plus exact section probes.** Three new `Shape` methods turn existing 3D geometry (an imported STEP, a boolean result, a lofted body) back into a real `Sketch` that composes with `.extrude` / `.revolve` / `.reflect` and every 2D op. `shape.sectionSketch(plane)` computes an exact planar cross-section (`BRepAlgoAPI_Section`) and returns one sketch whose first loop is the outer boundary and whose remaining loops are holes — a boss with a through-hole sections as an outer rectangle plus an exact circle, so a single `.extrude(d)` yields the solid section. Circular section curves stay exact `bulgeArc` commands (a sectioned Ø10 hole is still Ø10, not a polyline); only splines/ellipses are chord-flattened under `opts.curveTolerance`. The `plane` argument accepts `'xy'|'xz'|'yz'`, `{ plane, offset }`, or any `{ origin, normal }`. `shape.faceSketch(face)` walks a planar face's wires (outer + holes) and places them in the face's own 2D frame; it takes the same selector as `.shell()`/`.hole()` — canonical name, label, `FaceQuery`, or `@kc[...]` ref — and round-trips (`faceSketch('top').extrude(t)` rebuilds the cross-section prism). `shape.silhouette(direction)` returns the orthographic outline via OCCT HLR (a cylinder along its axis silhouettes as a circle, across its axis as a rectangle). All three are async and lower at capture time. New diagnostic codes `feature.section.plane-misses-body` and `feature.face-sketch.non-planar`. Catalogue grows 276 → 278.
- **Actionable diagnostic for un-awaited async Sketch producers.** `shape.sectionSketch(...)`, `.faceSketch(...)`, and `.silhouette(...)` return `Promise<Sketch>`; chaining a Sketch method directly on the un-awaited call — `part.sectionSketch('xy', 5).extrude(3)` — used to fail with a cryptic `TypeError: sec.extrude is not a function`. It now throws `feature.async-result.missing-await`, naming the exact fix (`(await shape.sectionSketch(...)).extrude(...)`). `await`, `.then`, `.catch`, `.finally`, and `Promise.all` are unaffected. New diagnostic code. Catalogue grows 278 → 279.
- **`inspect({ of: 'section', ... })` — numeric cross-section evidence.** Returns `area`, `perimeter`, `loopCount`, `holeCount`, and the 2D `bbox` at a plane (`plane`, or shorthand `{ axis, at }`), or `stack: { from, to, count, axis? }` for a dense scan; the response reports `minAreaIndex`/`minAreaPosition`, which locates a neck, waist, or thinnest wall along an axis. It uses the same section geometry as `sectionSketch`, so a probe and a derived sketch can never disagree.
- **Example + cookbook.** `examples/section-gasket.kcad.ts` (+ `.run.ts`) sections a two-hole plate and extrudes a matching 3 mm gasket, then stack-scans a stepped shaft to find its neck; `cookbook/snippets/section-sketch-gasket.md` makes the pattern reachable from `lookup_cookbook`. `kernelcad-authoring` skill documents the three methods and the section probe.
- **Typed script params — boolean/choice/string beyond numeric `param()`.** `param(name, defaultValue, meta?)` now infers a `choice` kind when `defaultValue` is a string AND `meta.choices` is given (`param('Screw', 'M4', { choices: ['M3','M4','M5'] })`), or a plain `string` kind for any other string default (`param('Label', 'KCAD', { maxLength: 24 })`); numeric and boolean params are unchanged. Choice/string params return a new `TypedParamRef` (not the numeric `ParamRef` — no `.add()`/`.multiply()` chain, cannot coerce to a number) that resolves EAGERLY: read `.value` in script logic (`if (hasLid.value)`, `diameters[screw.value]`, `sketch.text(label.value)`) instead of a symbolic re-evaluation — see the design spec for why eager resolution beats a `select()` AST node for this case. `ParamTable`/`set_param`/`inspect({ of: 'params' })` (now `{ name, type, value, defaultValue, choices?, min?, max?, description? }`) all validate the new kinds; a bad choice or an unchosen `meta.choices` surfaces `feature.invalid-args` (hint `invalid-args.param.choice-invalid`), an over-length string surfaces `invalid-args.param.value-out-of-range`. `kinematic.sweepTolerance` now rejects sweeping a non-numeric param up front with `invalid-args.param.type-mismatch` instead of 64 confusing per-combo failures — this also caught and fixed a pre-existing env-var leak in the sweep's validation pre-check that could silently break the mechanism gates on later combos. The Studio params panel renders a checkbox (boolean), a `<select>` (choice), and a text input (string, commits on blur/Enter). New example `examples/params/typed-enclosure.kcad.ts` and cookbook entry `typed-params-boolean-choice-string`. **Boolean `ParamRef`s ALSO gain an eager `.value` snapshot** (previously `undefined` — a boolean `param()` had no way to drive script control flow like `if (hasLid.value) {...}`; it stayed a purely symbolic `ParamRef` usable only via `enabled:` opts). `makeParamRef`/`ParamRef`'s constructor take an optional `value` (set only for the `'boolean'` type; numeric `ParamRef.value` stays `undefined` — numeric params remain purely symbolic). **`set_param` now validates `new_value` against the param's declared kind BEFORE rewriting the source** — a non-boolean for a boolean param, a non-string for a string param, or a value outside `choices` is refused (`ok: false`, no `code` in the result) instead of silently corrupting the default and only failing later on re-evaluation. **`inspect({ of: 'params' })` now accepts `{ file? }` / `{ code? }`** like every other `inspect` reader — it used to ignore both and only read whatever session `evaluate_script` last left active, returning `{ params: [] }` when called cold.
- **Automatic dimensioning and GD&T on `svg-drawing` — `options.autoAnnotate`.** The exporter reads the exported solid and draws a sheet a shop can quote from: datums A/B/C (A = largest planar face, B / C = largest planar faces orthogonal to it that a hole pattern is located from, with documented tie-breaks), grouped hole callouts for simple, counterbored and countersunk holes (`4× ⌀6.5 THRU`, `⌀5.5 THRU ⌴⌀10 ▾ 3`) with a position frame to the datums stacked under each (`⌖ ⌀0.1 A B C`), hole positions dimensioned from the datum planes, overall size, grouped radius and chamfer callouts (`4× R5`, `2× 1.5 × 45°`, legs measured from the geometry), flatness on A, and an `ISO 2768-mK` general-tolerance cell beside the title block. Values come from ISO 2768-1 / -2 for `tolerance: 'ISO2768-f' | 'ISO2768-m' | 'ISO2768-c'`; `include` narrows the families and `datums` pins letters to faces. Leader callouts are placed by testing rendered candidates against drawn geometry, labels, captions and section indicators. The export returns a placement report (`drawing_report` over MCP, `drawingReport` from `runAndExport` and `kernelcad export --json`) with placed / overlapped counts, the datum frame and every annotation drawn; residual collisions still warn `drawing.annotation.overlap`.
- **`shape.datum(label, face)` and `shape.tolerance({ type, value, face | edge, datums?, modifier? })`.** GD&T declared on the feature graph as virtual records bound to the declaring shape; both return the same shape. The drawing exporter collects declarations made on the exported shape or anything feeding it. A declared datum pins its letter; a declared tolerance replaces the automatic tolerance of the same type on the same feature (a grouped hole pattern is one feature). Declarations draw with or without `autoAnnotate`. Malformed declarations throw `feature.invalid-args` at capture.
- **Oblique section views.** `options.sections[].plane: { origin, normal }` accepts any non-zero normal: a real rotated half-space cut, an auxiliary true-shape section cell with the cut faces hatched, and a trace indicator on the standard view closest to edge-on. Axis-aligned planes are unchanged.
- **`kernelcad export --options <json>`.** Passes the per-format options bag from the CLI (e.g. `'{"autoAnnotate":true}'`) and prints the drawing placement summary.
- 2 new diagnostic codes: `drawing.auto.datum-ambiguous`, `drawing.auto.hole-unclassified` (both warn). Catalogue grows from 276 to 278. Example: `examples/drawings-auto/`; cookbook entry `drawing-auto-dimension-gdt`.
- **`drawing_to_cad` MCP tool: engineering drawing PDF to an editable part.** Reads a vector drawing page deterministically, with no vision model: stroked paths with their width and dash, and positioned text (via `pdfjs-dist`). Lines are classified as visible, hidden, center, dimension, extension or leader. The title block gives the scale, the units and the projection symbol. Front / top / side views are identified by projection alignment, third- or first-angle. Dimension text (`⌀`, `R`, `4×`, `±`, `THRU`, `▾ depth`) is tied to its lines, and stated values win over drawn lengths. The part is rebuilt as the non-rectangular view's silhouette extruded by the depth an orthogonal view shows, or as a turned part revolved from its half-silhouette, plus holes from `⌀` circles with THRU or hidden-line depth. Output is a `.kcad.ts` with role-named params (`width`, `thickness`, `holeDia`, `hole1X`, `dia1`, `step1Length`, …) and an assumption ledger (the same `resolve_assumptions` ledger as photo tracing): stated dimensions are `visible`, symmetry-placed and measured values `inferred`, defaults `assumed`, an unstated depth `missing`. A dimension that disagrees with the linework keeps its value and records `disagreement { stated, measured }`. The result is evaluated and re-projected through the `svg-drawing` view stage for a `fidelity.verdict` (match / partial / mismatch / failed) from extents, hole diameters and per-view silhouette IoU. New diagnostics: `reference.drawing.raster-only` (scanned page; points at `trace_from_image`), `reference.drawing.view-ambiguous`, `reference.drawing.dimension-unassociated`, `reference.drawing.depth-missing`. Example: `examples/drawing-to-cad/`.
...
- **`mesh_to_features` — mesh or scan to an editable, verified feature tree.** Hand it an STL (binary or ASCII), OBJ or 3MF of a mostly prismatic mechanical part (`{ file }` or base64 `{ data }`) and it returns a readable `.kcad.ts` instead of the faceted solid `lib.fromSTL` gives: named `param()`s, a revolve for concentric round stacks or extruded profiles (exact lines, arcs and circles fitted from band sections, near-orthogonal directions and near-round values snapped; a rectilinear outline becomes `length`/`width` plus per-block extents, with corner rounds as `.fillet(radiusParam, edges)` or, if that reading does not verify, param-driven `.tangentArc` — never literal `threePointsArc` coordinates), `.hole()`/`.holes()` for through, blind and counterbored bores — axial or drilled into side faces, positioned against the entry face centroid the kernel will actually use — `.cutout()` pockets, `.fillet()` for constant-radius edge blends (each radius measured from the arc tangent to the two faces of the sharp edge, snapped and ledgered, edges grouped by radius and selected with the shortest exact edge query; variable-radius blends and chamfers are rejected and stay approximate), and boolean subtractions for bores no drilling feature can reach. The pipeline is deterministic: weld and watertight check, noise-aware plane and cylinder segmentation, extrusion-axis choice, then up to four fit/emit/verify passes that EVALUATE the script and compare it with the mesh. `fidelity` reports volume IoU (column ray casting), max and RMS surface deviation, and a `verdict` computed from those numbers — `faithful` needs IoU >= 0.98, max deviation <= max(0.25 mm, 0.1 % of the diagonal), a watertight mesh and no unmatched region; thresholds are overridable and always echoed. Surface the script misses (freeform, tilted planes, side bosses) is listed in `unmatchedRegions`. The returned `ledger` classifies every measurement, snap, hole classification and drilling-side choice as `visible`/`inferred`/`assumed`/`missing`, with dimension facts keyed by param name so `resolve_assumptions` on the written `<out>.ledger.json` produces `set_param` overrides. CLI: `kernelcad reconstruct <mesh> -o <out.kcad.ts>` (`--strict` exits 1 unless faithful). Round trips of exported kernelCAD models (4-hole plate, cross-drilled L-bracket, flanged cylinder with a bolt circle, counterbored spacer, `examples/bracket-with-hole.kcad.ts` with every edge rounded, a plate with 3 mm and 1 mm rounds, a stepped block whose `length`/`width` and `filletRadius` re-evaluate under `set_param`) come back faithful with max deviation <= 0.05 mm; examples in `examples/mesh-to-features/` (an angle bracket, and an end stop whose two radii return as fillet groups), cookbook entry `rebuild-editable-model-from-mesh`.
- **3 new diagnostic codes.** `reference.mesh.not-watertight`, `reference.mesh.low-fidelity`, `reference.mesh.freeform-region-unmatched`. Catalogue grows from 276 → 279.
- **`Sketch.extrude` accepts a `param()` depth.** The depth was serialized with `String(depth)`, so passing a `ParamRef` failed at lowering; it now routes through the same Editable path as every other scalar and re-evaluates on `set_param`. A plain number produces the identical record.
- **`meshDeviation` reports `rmsDeviationMm`** alongside the max and mean, and answers nearest-triangle queries from a grid (same distances, far less work on dense meshes).
- **Multi-group `fillet([{ edges, radius }])` accepts `param()` radii.** The group radius was stored as the raw ParamRef and the fillet was refused at lowering; it is now resolved and re-evaluated on `set_param`.
- **Curves-surfacing primitives.** Three generic kernel ops, no new MCP tool family:
  - `curveBridge(a, b, { continuity: 'G1'|'G2', ends?, tension? })` / `Curve3D.bridge` — infer end frames from two existing curves and emit a degree-5 Hermite (reuses `hermiteG2`). Continuity error at both joins is `< 1e-6`. Degenerate ends → `feature.curve-bridge.degenerate-end`.
  - `await surfaceIntersection(a, b)` — exact face–face / surface–surface section via OCCT `BRepAlgoAPI_Section`, returned as `Curve3D[]` usable by sweep / `projectCurve` / trim. Empty section → `feature.surface-intersection.none`. Cylinder ∩ plane at 30° is the analytic ellipse; two equal-radius cylinders (Steinmetz) yield closed section loops.
  - `Sketch.loft(..., { rails })` — guide-rail loft via `BRepOffsetAPI_MakePipeShell` (first rail = spine, second = auxiliary `SetMode_5`). A rail that misses a section, or a third rail, emits `feature.loft.rail-miss`. Zero rails keep the existing ThruSections path.
- **3 new diagnostic codes.** Catalogue 276 → 279.
- Example `examples/curves-surfacing/handle-and-tee.kcad.ts` (rail-loft grip, G2 crown, pipe-tee weld seam). Cookbook snippet `g2-bridge-and-intersection-curve`.
- **Surface quality — continuity, curvature, zebra.** `inspect({ of: 'continuity' })` samples each shared edge for G0 position gap (mm), G1 normal jump (deg) and G2 curvature difference, then classifies the edge `G0` / `G1` / `G2` / `broken` with the worst-sample XYZ. A box corner is G0 only (90°); a default fillet-to-face edge is G1 not G2; a G2 blend (`continuity: 'G2'` on NURBS-adjacent faces) classifies G2. `inspect({ of: 'curvature' })` reports per-face min/max/mean Gaussian and mean curvature, inflections and spikes (sphere K = 1/r²; cylinder K = 0 and |H| = 1/(2r)). `render_preview({ overlay: 'zebra' | 'curvature' | 'continuity' })` draws those numbers through the existing offline renderer (zebra from vertex normals vs a view-aligned stripe direction; curvature as vertex colours on the FEA heatmap path; continuity edges coloured by class). CLI: `kernelcad inspect continuity <file>` / `kernelcad inspect curvature <file>`. New `inspect` diagnostic group: `inspect.continuity.g1-break`, `inspect.continuity.broken`, `inspect.curvature.spike`. Catalogue 276 → 279.
- **`svg-drawing` feature dimensions + GD&T annotation kinds + section views.** `export({ format: 'svg-drawing', options: { annotations } })` gains five feature-aware annotation kinds alongside the existing generic ones: `hole` (standard callouts — `⌀6.5 THRU`, blind `⌀6.5 ▾ 10`, `⌴`/`⌵` counterbore/countersink suffixes, `<count>× ` pattern prefix), `fillet` (`R<r>`), `chamfer` (`<size> × <angle>°`, leg size author-supplied — it isn't recoverable from a bare edge query), `datum` (ASME square datum-feature symbol + leader), and `fcf` (feature control frame: type symbol · toleranced value · datum references, for `position`/`flatness`/`perpendicularity`/`parallelism`/`concentricity`/`cylindricity`). The existing `linear` kind gains `tol` (`± 0.1`, `+0.2/−0.05`, or a fit class string like `'H7'`). All five share the existing per-view leader-rotation stacking, so they fan out from other callouts on the same view automatically.
- **`options.sections` — real section views.** A real OCCT half-space boolean cut (not a render-time visual clip) through the assembled body, projected through the same axis-aligned camera as the standard views, with the true cross-section hatched at 45° (a hole the plane slices through renders as a hole in the hatch, `fill-rule="evenodd"`). A cutting-plane indicator (dashed line, arrows, letter) draws on the view where the plane appears edge-on; the sheet grows taller to fit the new section cell — standard views are pixel-for-pixel unaffected. Only axis-aligned planes (`'xy'|'xz'|'yz'`, or an `{ origin, normal }` within ~2.5° of a world axis) are supported; oblique planes fail with `feature.invalid-args` rather than being approximated. A plane missing the body fails with `drawing.section.plane-misses-body`.
- **`drawing.annotation.overlap` — real, non-fatal text-box collision check.** After rendering, every axis-aligned annotation label's approximate bounding box is compared pairwise across different annotations; an overlapping pair does not fail the export but emits one `warn` diagnostic naming every pair (surfaced via `runAndExport`'s diagnostics array). Rotated labels are skipped rather than mis-estimated.
- 4 new diagnostic codes (new `drawing` group): `drawing.datum.unresolved`, `drawing.tolerance.feature-unresolved`, `drawing.section.plane-misses-body`, `drawing.annotation.overlap` (warn). Catalogue grows from 248 → 252. GD&T is authored via `options.annotations` rather than new `Shape.datum()`/`Shape.tolerance()` capture-graph methods — documented as a deliberate scope decision in the `kernelcad-drawings` skill.
- **Assumption ledger for reference-photo modeling.** `trace_from_image` now returns a `ledger` alongside `features`/`diagnostics`: every fact from the trace is classified `visible` (opencv contour, deterministic), `inferred` (vision-LLM labeled, self-reported confidence), `assumed` (caller-supplied prior, e.g. wall thickness), or `missing` (a scale anchor was never supplied — never fabricated). New `scaleAnchor`/`priors`/`validate` inputs let a caller ground the pixel-to-mm scale or record category-norm priors; `validate: 'error'` turns an open `missing` fact into a hard failure instead of a warning. New MCP tool `resolve_assumptions({ ledgerPath, resolutions })` confirms/overrides open facts in the persisted `<model>.ledger.json`, rewrites it, and returns `paramOverrides` for `set_param`. New diagnostic codes: `reference.assumptions.unresolved`, `reference.assumptions.ledger-not-found`, `reference.assumptions.unknown-resolution-id`. Catalogue grows from 248 → 251. `kernelcad-trace-from-image` skill and the `kernelcad-from-reference` orchestrator's gate table updated; new `resolve-photo-trace-assumptions` cookbook entry; example under `examples/from-reference/assumption-ledger/`.
- **Structural FEA gate — `shape.feaStudy(...)`, `run_fea`, `fea_summary`.** An agent can now ask "will this part hold this load?" and get solver evidence instead of a guess. `shape.feaStudy({ material, fixed, loads, meshSize?, minSafetyFactor? })` declares a linear-static study as a virtual record bound to one solid; `fixed` and `loads[].faces` take the existing `FaceQuery` / `@kc[...]` selector vocabulary and `force` is the TOTAL newtons on the selected faces. `run_fea` meshes the solid with quadratic tetrahedra (gmsh), solves a `*STATIC` step with CalculiX, and returns peak von Mises stress (MPa), peak displacement (mm), the minimum safety factor against the material yield, per-region hot spots named by `@kc[...]` face ref, mesh-quality trust flags, an equilibrium residual (reaction vs applied load), and banded stress-heatmap PNGs rendered through the existing offline `render_preview` pipeline. `fea_summary` is the read-only companion: a stored result, the toolchain's availability with its install command, and the material table. Declaring `minSafetyFactor` turns the study into an enforcement gate that fails `evaluate_script` / `kernelcad evaluate` through the same seam as the `dfmSpec` gates; a study without one costs nothing at evaluate. The solver stack is external and open source (`calculix-ccx` + `pip install gmsh==4.15.2`) — nothing is bundled, and an absent toolchain fails loudly with `fea.solver.unavailable` rather than reporting a pass. `KERNELCAD_FEA_GATE=off` skips the solve and says the declared margin is UNVERIFIED.
- **FEA material grades with E, Poisson's ratio and yield.** `mild-steel`, `aluminum-6061`, `pla`, `petg`, `abs`, `nylon`, in solver units (MPa). Stiffness and yield for the five grades with a bulk-catalog row are read LIVE from `MATERIAL_CATALOG`; this table declares only Poisson's ratio (a genuinely new fact) and the `nylon` row (no catalog row exists for it). An unknown grade is refused with the valid list — never guessed. Measured properties go in as explicit `{ E, nu, yield }`.
- **5 new diagnostic codes.** `fea.safety-factor.below-min`, `fea.mesh.quality-low`, `fea.solver.unavailable`, `fea.study.fixed-unresolved`, `fea.study.load-unresolved`. Catalogue grows from 248 to 253; the new `fea` diagnostic group joins the vocabulary.
- **New `kernelcad-fea` skill + `examples/fea/shelf-bracket-fea.kcad.ts`.** The example is an L-bracket whose stress riser sits in a root fillet — a feature no beam formula names — with the run commands and measured expected output in its header, including the variant that fails the gate. A cookbook entry (`fea-study-safety-factor-gate`) makes the pattern reachable from `lookup_cookbook`.

- **Close the physical print loop: `export` gains `format: 'gcode'`, plus a `send_to_printer` tool.** `export({ target: 'model', format: 'gcode', options: { printer, material, layerHeight, infill, supports } })` meshes the target shape, bbox-gates it against the selected printer profile's bed size, then shells out to a real slicer CLI (OrcaSlicer, or PrusaSlicer where installed — detected via `KERNELCAD_SLICER` or PATH) to produce real G-code; no placeholder output is ever returned. On success the result carries `gcodeStats` parsed from the slicer's own comments (`estimatedPrintTimeSeconds`, `filamentUsedGrams`, `filamentUsedMeters`, `layerCount`, `maxZMm`). New `send_to_printer` tool uploads the G-code to a real network printer and starts the print: OctoPrint (`POST /api/files/local`), Moonraker (`POST /server/files/upload`), or Bambu Lab LAN mode (FTPS implicit-TLS upload + MQTT print-start). `dryRun`/`dry_run` validates connectivity/auth only. CLI: `kernelcad export gcode <file> -o <out.gcode>` and `kernelcad print send <gcode-file> --protocol <p> --host <h> ...`. New skill `kernelcad-print`. 4 new diagnostic codes: `export.gcode.exceeds-bed`, `export.gcode.slicer-unavailable`, `tool.send-to-printer.unreachable`, `tool.send-to-printer.upload-failed`. Catalogue grows 248 → 252.
- **Trace-guided repair — `repair_script` MCP tool + `why_did_this_fail` repair plan.** A failed evaluation now leads to a bounded, applied fix instead of a re-read of the whole script. Every captured feature is bound to the call site that authored it (`FeatureRecord.scriptLocation`, resolved from a real call-site frame through the transpile source map and the module-ism normalizer, so the line is a coordinate in the file the agent edits). `why_did_this_fail` returns that `trace` (call site, AST node range, statement range, diagnostics, inputs, dependents, paramRefs), a `repairRegion` — the failing feature's statement plus its direct inputs' statements plus the `param()` lines it reads, and nothing else — and ordered `candidates`, each a real AST-anchored patch with a predicted effect and the geometry it was derived from. Eight diagnostic kinds produce candidates: oversized fillet/chamfer (`feature.edge-feature.short-edges-skipped`, ladder under half the shortest adjacent edge), boolean and hole no-ops (`feature.subtractive-noop`), empty intersections (`feature.intersection-empty`), dead edge selectors (`feature.selection.no-match`), unknown labels (`feature.label.unknown-name`), zero emboss depth (`feature.emboss-text.depth-zero`), and out-of-range face anchors (`feature.face.invalid-uv-anchor`). Everything else returns `candidateStatus: 'no-automatic-candidate'` with the region — an honest gap, not an invented patch. `repair_script({ file | code, diagnostic, strategy: 'apply-first' | 'try-all' | 'dry-run', max_attempts })` applies candidates one at a time, re-evaluates after each, and keeps the first that clears the diagnostic without new error codes; it returns `new_code` + a unified `diff` + before/after health maps, and refuses any patch that escapes the region or whose anchor text has drifted. New `kernelcad-repair` skill documents the loop.
- **`kernelcad evaluate --trace-out <json>`.** Persists the same per-feature trace to disk, so a CI failure can be read after the fact without re-running the script.
- **Repair example + cookbook entry.** `examples/repair/` ships three deliberately broken models (oversized fillet, hole anchored off its plate, cutter placed away from its body) and `run-repair-example.ts`, which runs `evaluate_script` → `why_did_this_fail` → `repair_script` dry-run → apply-first → `evaluate_script` through the MCP dispatcher and prints the diagnostics, the patch, and the clean result. `npx tsx examples/repair/run-repair-example.ts`; the README carries the captured output. The `repair-oversized-fillet` cookbook snippet (the repaired, evaluable model) is what `lookup_cookbook` returns for "repair failing fillet". Hole candidates now keep one bore radius of wall instead of parking the bore tangent to the face edge.
- **4 new diagnostic codes.** `tool.repair.no-candidate`, `tool.repair.out-of-region`, `tool.repair.exhausted`, `tool.repair.source-drift`. Catalogue grows from 248 → 252.
- **Capture-time emboss diagnostics now name their feature.** `sketch.text.empty-content`, `feature.emboss-text.depth-zero`, and `feature.face.invalid-uv-anchor` are validated before the record exists, so they used to reach agents with no `featureId` and could not be joined back to a modeling step. The lowerer now stamps the owning id when it surfaces them.

- **`kinematic.checkStaticHold` — actuator vs gravity holding check.** New `kinematic.*` entry point, wired as `verify({ check: 'static-hold' })`: per actuated joint declaring `actuator: { torqueNm | forceN }` on `arm.revolute(...)`/`arm.prismatic(...)`, samples a pose grid across the joint's declared range (or an explicit `opts.pose`) and computes the gravitational torque/force the actuator must supply to hold the downstream mass still, using real OCCT mass properties from the loaded parts' geometry + declared density/material. Reports the worst pose, required torque/force, and margin percent against the declared capacity. New diagnostics: `assembly.joint.static-hold.exceeded` (error, actuator undersized), `assembly.joint.static-hold.margin-low` (warn, under the requested safety margin but not exceeded), `kinematic.static-hold.no-actuator-declared` (error, no silent default). `RevoluteJointOpts`/`PrismaticJointOpts` gain an `actuator` field.
- **`kinematic.sweepTolerance` — parameter-space tolerance sweep.** New `sweep_tolerance` MCP tool + `kinematic.sweepTolerance({ code|file, params, gates })`: declares one or more `param()` names as `{ values }` or `{ min, max, steps }`, expands the cartesian product (capped at 64 combos — `kinematic.sweep-tolerance.combo-cap-exceeded` fires and truncates past the cap), and re-evaluates the script per combo via the existing `set_param` + `evaluate_script` path. Runs the interference / mounting-hole / joint-axis gates per combo by default (one `validateAssemblyWithMates` call), plus reachability when `gates.reachable` names a target. Returns a pass/fail envelope table (`results[]`) and `firstFailure[gateName]` — the first combo where a gate broke.
- **4 new diagnostic codes.** `assembly.joint.static-hold.margin-low`, `assembly.joint.static-hold.exceeded`, `kinematic.static-hold.no-actuator-declared`, `kinematic.sweep-tolerance.combo-cap-exceeded`. Catalogue grows from 248 → 252.
- **`diff_geometry` MCP tool.** Material-level delta between two versions of a model, for the case `diff_scripts` cannot resolve: a volume delta is ambiguous about WHERE material moved (a grown boss and a deepened pocket report the same magnitude; a displaced part reports zero). Baseline is `{ baseFile }` / `{ baseCode }`; the revised side is either another script or the same script re-lowered with `{ params }` overrides. Per matched body it returns `addedMm3` = volume(revised − base), `removedMm3` = volume(base − revised), `commonMm3` = volume(base ∩ revised) from OCCT booleans, exact bbox with min/max/extent deltas, face/edge/hole count deltas (hole counts reuse the cylindrical-hole detector), a two-sided discrete Hausdorff `maxDeviationMm`, and a `verdict` — `identical` | `moved` | `resized` | `topology-changed` — which is the field to branch on. Bodies pair by name with a positional fallback; leftovers raise `diff.body.unmatched`. `{ render: true }` adds an added-green / removed-red overlay PNG through the `render_preview` pipeline and fails open. `diff_scripts` now sets `deeperDiffAvailable` when its own numbers cannot say what physically changed, pointing at this tool instead of at a render.
- **UsdPhysics robot export (`format: 'usd-isaac'`).** `export({ target: 'model', format: 'usd-isaac' })` writes an ASCII USD (`.usda`) physics stage a GPU simulator imports directly: a `PhysicsArticulationRootAPI` root; one rigid body per link placed at its mate-solved pose, with mass, centre of mass, and principal inertia (`diagonalInertia` + `principalAxes`, positive-definite by construction via the MJCF exporter's regularization); a `PhysicsFixedJoint` / `PhysicsRevoluteJoint` / `PhysicsPrismaticJoint` per fastened / revolute / prismatic mate with a token axis, two-sided joint frames, and limits; `UsdPreviewSurface` materials from each part's `.finish()` / `.material()` / `.color()` / `{ material }`; and one `meshes/<part>.usda` mesh layer per link, referenced by the visual and collision prims. Joint drives are emitted only when declared in `options.drives` (never invented zero-gain drives); `options.collisionApproximation` picks `convexHull` or `convexDecomposition`. Fails closed with `export.usd.joint-unsupported` (planar / cylindrical / pin_slot / ball mates) and `export.usd.mass-missing`; warns with `export.usd.pose-unsolved` when the mate graph does not solve.
...
- **Rational NURBS weights via `Geom_BSplineSurface_2` (E1).** `add_surface({ kind: 'nurbs', weights })` now passes a weight grid through to the OCCT WASM binding (replicad-opencascadejs kcad-v0.23.2), enabling true rational B-spline surfaces (NURBS, not just B-splines). Useful for exact conic sections — a uniform knot vector + arc-weight grid produces a geometrically exact cylinder or sphere rather than a polynomial approximation. Surfaces without a weight array fall back to the previous non-rational path; no API break.
- **`Surface.trimTo(by)` and `Surface.split(by)` — planar trim/split (E2).** A surface can now be cut by a planar cutter (`Surface` or `Shape`). `trimTo` removes the smaller piece; `split` removes the smaller piece and emits a `feature.surface-trim.split-deferred` diagnostic (returning both pieces in a single face is deferred to Slice F). Cutter is resolved by `add_surface` `byRef` field. Non-planar cutters emit `feature.surface-trim.non-planar`; misses emit `feature.surface-trim.no-intersection`.
- **`sew(surfaces, { tolerance?, requireClosed? })` → watertight solid (E3).** Stitches an array of `Surface` handles into a shell and optionally closes it into a solid (`requireClosed: true`). Backed by `BRepBuilderAPI_Sewing` + `BRepBuilderAPI_MakeSolid`. Open shells emit `feature.surface-sew.open-shell` (warning, not error) unless `requireClosed` is set, in which case the feature fails. The `enclosure-half` eval task (6 planar patches → trim → sew → 24000 mm³ STEP-exportable solid) is the acceptance gate.
- **`Shape.draft(angleDeg, { face, neutralPlane?, pullDir? })` — draft angle for moulded parts (E4).** Tilts analytic faces of an existing solid by a draft angle relative to a pull direction, backed by `BRepOffsetAPI_DraftAngle`. If `neutralPlane` is omitted it is derived from the face's own bounding box midplane and `feature.draft.neutral-plane-derived` is emitted. Failure to apply the angle emits `feature.draft.failed`. Curved faces (fillets, NURBS patches) are rejected with `feature.surface-trim.non-planar` until Slice F adds a general-face draft path.
- **6 new diagnostic codes.** `feature.surface-trim.no-intersection`, `feature.surface-trim.non-planar`, `feature.surface-trim.split-deferred`, `feature.surface-sew.open-shell`, `feature.draft.failed`, `feature.draft.neutral-plane-derived`. Catalogue grows from 227 → 233.
- **Deferred to Slice F.** Face-face blends, surface offset, curved-patch trim, rational 3D curves, and union of sewn shell into an existing solid are out of scope for this slice.
