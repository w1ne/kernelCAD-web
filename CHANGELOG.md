...
- **Exploded assembly views + BOM balloons.** Pure `explodedPoses(arm, { factor, mode: 'radial' | 'mate-axis' })` pulls parts apart along the mate/joint spanning tree (cumulative down the tree) or radially from the assembly centroid; spacing is part size × factor, and existing BREP interference detection iteratively increases spacing so a stacked assembly does not interpenetrate in the exploded pose. `render_preview({ explode: { factor, mode } })` and `kernelcad render --explode <factor> --explode-mode <mode>` compose those translations onto the existing mesher — no new renderer. `export({ format: 'svg-drawing', options: { exploded, balloons, partsList } })` draws an exploded isometric cell, item balloons numbered from the BOM, and a parts-list table (item, name, qty, material) above the title block; overlap uses the existing `drawing.annotation.overlap` check. New diagnostics: `render.explode.no-assembly`, `drawing.balloons.bom-unavailable`. Catalogue 278 → 280. No `arm.explode()` authoring hook (render/export consume `explodedPoses` directly; animation interpolation is out of scope). Example: `examples/exploded/enclosure.kcad.ts`; cookbook `exploded-assembly-drawing`.
- **`Shape.wrapTexture(imageRef, projection)` — projected bitmap textures.** Wraps an image onto a shape's surface with UVs derived from a projection strategy instead of requiring hand-authored UVs: `flat({ onto })` (planar onto `'xy'|'xz'|'yz'`), `cylinder({ axis })` (labels/cans/tubes — `u` = angle, `v` = height), `sphere()` (longitude/latitude), `box()` (six-sided triplanar). UVs are computed from the shape's FINAL world-space vertex positions at mesh/export time (`computeProjectedUVs`), so a wrap survives any `.translate()`/`.rotate()`/`.fillet()` applied after the `wrapTexture()` call — unlike UVs baked in eagerly, which a later transform would leave stale. Render-only: geometry is unchanged; an engraved/embossed image still goes through `projectCurve` + a sketch-based emboss/engrave feature. `export({ format: 'glb' })` now carries the real texture image + a `TEXCOORD_0` accessor for wrapped parts — GLTFExporter (three-stdlib) has no Node canvas for its own image-encoding path, so the image is embedded via a `@gltf-transform/core` post-process keyed on the exported node name. New `src/shared/intent/textureProjection.ts` (pure UV math, unit-tested), `examples/can-label-wrap.kcad.ts`, and cookbook snippet `wrap-texture-can-label` (`lookup_cookbook("wrap a texture around a cylinder")`); documented in the `kernelcad-authoring` skill's Materials section.
- **BOM extraction — `inspect({ of: 'bom' })` + `export({ format: 'bom-csv' | 'bom-json' })`.** A real bill of materials over `assembly.model()`/`solvedModel()`, shared by one `computeBom` so both surfaces report identical numbers. Rows group by GEOMETRY/CATALOG IDENTITY, not by name — a pattern loop that places the same source shape under N distinct names collapses to one row with a real `quantity: N` (not a name-string heuristic); purchased parts (`lib.fetchPart`/`fetch_part` provenance) group by catalog id and carry `catalog: { id, vendor, partNumber, source, license }`, fabricated parts by a position-invariant volume/area/bbox fingerprint. Per-unit and total mass come from the part's declared `material`/`density` (`assembly.part(name, shape, opts)`); a part with neither gets `massG* : null` and a `bom.material.unassigned` diagnostic — never a guessed water-density mass. New `bom` diagnostic group: `bom.material.unassigned`, `bom.purchased.catalog-metadata-missing`. Catalogue grows 276 → 278. Retires the `bom-extraction` out-of-scope claim. New `examples/bom/panel-with-fasteners.kcad.ts` and `bom-ready-assembly` cookbook entry; documented in the `kernelcad-mcp` and `kernelcad-assemblies` skills.
